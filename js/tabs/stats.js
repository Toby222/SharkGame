SharkGame.Stats = {
	...
	switchTo() {
		...
		genStats.append(
			$("<p>")
				.html(
					"Real time since you began your journey:<br/><span id='gameTime' class='timeDisplay'></span>",
				)
				.addClass("medDesc"),
		);
		if (!firstTime) {
			genStats.append(
				$("<p>")
					.html(
						"Relative time since you came through the gate:<br/><span id='runTime' class='timeDisplay'></span>",
					)
					.addClass("medDesc"),
			);
			if (SharkGame.persistentFlags.scouting === false) {
				genStats.append(
					$("<p>").html(`Par: ${gateway.getPar()} minutes`).addClass("medDesc"),
				);
			}
		}
	...
	createDisposeButtons() {
		...
		SharkGame.ResourceMap.forEach((_resource, resourceId) => {
			if (
				...
				stats.bannedDisposeCategories.indexOf(
					res.getCategoryOfResource(resourceId),
				) === -1
			) {
				SharkGame.Button.makeButton(
					"dispose-" + resourceId,
	...
	updateDisposeButtons() {
		SharkGame.ResourceMap.forEach((_resource, resourceName) => {
			if (
				...
				stats.bannedDisposeCategories.indexOf(
					res.getCategoryOfResource(resourceName),
				) === -1
			) {
				...
				const disableButton =
					resourceAmount < amountToDispose || amountToDispose <= 0;
				let label =
					"Dispose of " +
				...
				if (
					button.html() !== label.replace(/'/g, '"').replace("<br/>", "<br>")
				) {
					button.html(label);
				}
	...
	onDispose() {
		...
		if (resourceAmount >= amountToDispose) {
			...
			const category =
				SharkGame.ResourceCategories[res.getCategoryOfResource(resourceName)];
	...
	updateTimers() {
		// update run times
		const gameTime = sharktext.formatTime(
			_.now() - SharkGame.timestampGameStart,
		);
				...
				if (SharkGame.ResourceMap.get(resourceId).income) {
					...
					$.each(income, (incomeKey, incomeValue) => {
						...
						if (SharkGame.BreakdownIncomeTable.get(resourceId)) {
							realIncome =
								SharkGame.BreakdownIncomeTable.get(resourceId)[incomeKey];
						} else {
							return true;
						}
						...
						newValue = !sharktext.shouldHideNumberOfThis(resourceId)
							? "<div style='text-align:right'>" +
								sharktext.beautify(res.getResource(resourceId)).bold() +
								"</div>"
							: "";
						if (cell.html() !== newValue.replace(/'/g, '"')) {
							cell.html(newValue);
	...
	updateTotalAmountTable() {
		SharkGame.ResourceMap.forEach((_resource, resourceId) => {
			...
			if (
				totalResource > 0 &&
				res.getCategoryOfResource(resourceId) !== "hidden"
			) {
	...
	createIncomeTable() {
		...
		SharkGame.ResourceMap.forEach((generatorData, generatorName) => {
			if (res.getTotalResource(generatorName) > 0 && generatorData.income) {
				...
				for (const incomeResourceName in generatorData.income) {
					...
					if (
						SharkGame.PlayerResources.get(incomeResourceName) <
							generatorData.income[incomeResourceName] &&
				...
				$.each(generatorData.income, (incomeKey, incomeValue) => {
					if (
						world.doesResourceExist(incomeKey) &&
						SharkGame.FlippedBreakdownIncomeTable.get(incomeKey) &&
						incomeValue !== 0
					) {
						if (SharkGame.Settings.current.switchStats) {
							// Switch it!
		...
		drawnResourceMap.forEach((headingData, headingName) => {
					...
					$("<td>")
						...
						.html(
							!sharktext.shouldHideNumberOfThis(headingName)
								? "<div style='text-align:right'>" +
										sharktext.beautify(res.getResource(headingName)).bold() +
										"</div>"
								: "",
						)
						.addClass(rowStyle)
			...
			$.each(headingData, (subheadingKey, _subheadingValue) => {
				const incomeKey = SharkGame.Settings.current.switchStats
					? headingName
					: subheadingKey;
				const generatorName = SharkGame.Settings.current.switchStats
					? subheadingKey
					: headingName;
				multipliers.upgrade.push(
					res.getMultiplierProduct("upgrade", generatorName, incomeKey),
				);
				multipliers.world.push(
					res.getMultiplierProduct("world", generatorName, incomeKey),
				);
				multipliers.aspect.push(
					res.getMultiplierProduct("aspect", generatorName, incomeKey),
				);
			});
			...
			$.each(headingData, (subheadingKey, subheadingValue) => {
				...
				if (SharkGame.BreakdownIncomeTable.get(generatorName)) {
					realIncome =
						SharkGame.BreakdownIncomeTable.get(generatorName)[incomeKey];
				} else {
					formatCounter++;
					return false;
				}
						...
						$("<td>")
							.html(
								!sharktext.shouldHideNumberOfThis(generatorName)
									? "<div style='text-align:right'>" +
											sharktext
												.beautify(res.getResource(subheadingKey))
												.bold() +
											"</div>"
									: "",
							)
				...
				if (SharkGame.Settings.current.grottoMode === "advanced") {
					addCell(
						[
							...
							baseIncomeChangeChar +
								sharktext.beautify(
									SharkGame.ResourceMap.get(generatorName).baseIncome[
										incomeKey
									] / SharkGame.persistentFlags.dialSetting,
									false,
									2,
					...
					if (multipliers.upgrade || counter === 0) {
						const upgradeMutiplier = res.getMultiplierProduct(
							"upgrade",
							generatorName,
							incomeKey,
						);
						if (upgradeMutiplier !== 1) {
							addCell(
								[
									res.UPGRADE_MULTIPLIER_COLOR,
									"x" + sharktext.beautify(upgradeMutiplier, false, 1),
								],
								multipliers.upgrade ? "inline" : undefined,
							);
						} else
							addCell(undefined, multipliers.upgrade ? "inline" : undefined);
					}

					if (multipliers.world || counter === 0) {
						const worldMultiplier = res.getMultiplierProduct(
							"world",
							generatorName,
							incomeKey,
						);
						if (worldMultiplier !== 1) {
							addCell(
								[
									res.WORLD_MULTIPLIER_COLOR,
									"x" + sharktext.beautify(worldMultiplier, false, 1),
								],
					...
					if (multipliers.aspect || counter === 0) {
						const aspectMultiplier = res.getMultiplierProduct(
							"aspect",
							generatorName,
							incomeKey,
						);
						if (aspectMultiplier !== 1) {
							addCell(
								[
									res.ASPECT_MULTIPLIER_COLOR,
									"x" + sharktext.beautify(aspectMultiplier, false, 1),
								],
								multipliers.aspect ? "inline" : undefined,
							);
						} else
							addCell(undefined, multipliers.aspect ? "inline" : undefined);
					}
					...
					if (resourceAffectMultiplier !== 1) {
						addCell(
							[
								res.RESOURCE_AFFECT_MULTIPLIER_COLOR,
								"x" + sharktext.beautify(resourceAffectMultiplier, false, 2),
							],
							"inline",
					...
					addCell(
						[
							...
							sharktext.boldString(
								Math.min(
									(realIncome / SharkGame.PlayerIncomeTable.get(incomeKey)) *
										100,
									100,
								).toFixed(0) + "%",
							),
		...
		if (drawnResourceMap.size && !_.isUndefined(incomesTable[0].children[0])) {
			...
			if (SharkGame.Settings.current.grottoMode === "advanced") {
				function tooltip($elt, html) {
					return $elt
						.on("mouseenter", () => $("#tooltipbox").html(html))
						.on("mouseleave", () => $("#tooltipbox").html(""));
				}
			...
			row.append(
				$("<td>").html("<span><u><b>TOTAL</b></u></span>").addClass("evenRow"),
			);

			incomesTable.prepend(row);
			SharkGame.Stats.incomeTableEmpty = false;
		} else {
			incomesTable.prepend(
				$("<tr>").append(
					$("<td>").html("<span><i><b>There's nothing here.</b></i></span>"),
				),
			);
	...
	toggleSwitch() {
		SharkGame.Settings.current.switchStats =
			!SharkGame.Settings.current.switchStats;
	...
	updateTableKey() {
		if (
			SharkGame.Settings.current.grottoMode !== "advanced" ||
			SharkGame.Stats.incomeTableEmpty
		) {
	...
	networkTextEnter(_mouseEnterEvent, networkResource = "nothing") {
		...
		if (
			_.some(
				generatorCondensedObject.genAffect,
				(category) => !$.isEmptyObject(category),
			)
		) {
			text +=
				sharktext.getResourceName(
			...
			if (!$.isEmptyObject(generatorCondensedObject.genAffect.increase)) {
				...
				$.each(
					generatorCondensedObject.genAffect.increase,
					(affector, degree) => {
						const amount = SharkGame.Settings.current.alwaysSingularTooltip
							? 1
							: res.getResource(affector);
						text +=
							sharktext.boldString(
								sharktext.beautify(degree * 100 * amount) + "%",
							) +
							" from " +
			...
			if (!$.isEmptyObject(generatorCondensedObject.genAffect.decrease)) {
				text +=
					...
					(addedAnyLabelsYet ? "<br>then " : "") +
					"IS DECREASED BY</span><br>";
				addedAnyLabelsYet = true;
				$.each(
					generatorCondensedObject.genAffect.decrease,
					(affector, degree) => {
						const amount = SharkGame.Settings.current.alwaysSingularTooltip
							? 1
							: res.getResource(affector);
			...
			if (!$.isEmptyObject(generatorCondensedObject.genAffect.multincrease)) {
				text +=
					"<span class='littleTooltipText'>" +
					(addedAnyLabelsYet ? "<br>then " : "") +
					"IS MULTIPLICATIVELY INCREASED BY</span><br>";
				addedAnyLabelsYet = true;
				$.each(
					generatorCondensedObject.genAffect.multincrease,
					(affector, degree) => {
						const amount = SharkGame.Settings.current.alwaysSingularTooltip
							? 1
							: res.getResource(affector);
						text +=
							sharktext.boldString(
								sharktext.beautify((degree ** amount - 1) * 100) + "%",
							) +
							" from " +
							sharktext.boldString(sharktext.beautify(amount)) +
							" " +
			...
			if (!$.isEmptyObject(generatorCondensedObject.genAffect.multdecrease)) {
				text +=
					"<span class='littleTooltipText'>" +
					(addedAnyLabelsYet ? "<br>then " : "") +
					"IS MULTIPLICATIVELY DECREASED BY</span><br>";
				addedAnyLabelsYet = true;
				$.each(
					generatorCondensedObject.genAffect.multdecrease,
					(affector, degree) => {
						const amount = SharkGame.Settings.current.alwaysSingularTooltip
							? 1
							: res.getResource(affector);
						text +=
							sharktext.boldString(
								sharktext.beautify((1 - degree ** amount) * 100) + "%",
							) +
							" from " +
							sharktext.boldString(sharktext.beautify(amount)) +
							" " +
		...
		if (
			_.some(
				generatedCondensedObject.resAffect,
				(category) => !$.isEmptyObject(category),
			)
		) {
			...
			if (!$.isEmptyObject(generatedCondensedObject.resAffect.increase)) {
				...
				$.each(
					generatedCondensedObject.resAffect.increase,
					(affector, degree) => {
						const amount = SharkGame.Settings.current.alwaysSingularTooltip
							? 1
							: res.getResource(affector);
			...
			if (!$.isEmptyObject(generatedCondensedObject.resAffect.decrease)) {
				text +=
					"<span class='littleTooltipText'>" +
					(addedAnyLabelsYet ? "<br>then " : "") +
					"ARE DECREASED BY</span><br>";
				addedAnyLabelsYet = true;
				$.each(
					generatedCondensedObject.resAffect.decrease,
					(affector, degree) => {
						const amount = SharkGame.Settings.current.alwaysSingularTooltip
							? 1
							: res.getResource(affector);
			...
			if (!$.isEmptyObject(generatedCondensedObject.resAffect.multincrease)) {
				text +=
					"<span class='littleTooltipText'>" +
					(addedAnyLabelsYet ? "<br>then " : "") +
					"ARE MULTIPLICATIVELY INCREASED BY</span><br>";
				addedAnyLabelsYet = true;
				$.each(
					generatedCondensedObject.resAffect.multincrease,
					(affector, degree) => {
						const amount = SharkGame.Settings.current.alwaysSingularTooltip
							? 1
							: res.getResource(affector);
			...
			if (!$.isEmptyObject(generatedCondensedObject.resAffect.multdecrease)) {
				text +=
					"<span class='littleTooltipText'>" +
					(addedAnyLabelsYet ? "<br>then " : "") +
					"ARE MULTIPLICATIVELY DECREASED BY</span><br>";
				addedAnyLabelsYet = true;
				$.each(
					generatedCondensedObject.resAffect.multdecrease,
					(affector, degree) => {
						const amount = SharkGame.Settings.current.alwaysSingularTooltip
							? 1
							: res.getResource(affector);

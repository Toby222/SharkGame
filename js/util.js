...
SharkGame.MathUtil = {
			...
			return constant
				...
				.times(
					difference
						.times(difference)
						.plus(difference)
						.plus(current.times(2).times(difference)),
				);
		} else {
			return (
				(constant / 2) *
				(difference * difference + difference + 2 * difference * current)
			);
		}
	},
	...
	linearMax(current, available, cost) {
		if (typeof current === "object") {
			return current
				.times(current)
				.plus(current)
				.plus(available.times(2).dividedBy(cost))
				.plus(0.25)
				.squareRoot()
				.minus(0.5);
		} else {
			...
			return (
				Math.sqrt(current * current + current + (2 * available) / cost + 0.25) -
				0.5
			);
		}
	},
	...
	uniqueCost(current, difference, cost) {
		if (typeof current === "object") {
			if (
				current.lessThan(1) &&
				current.plus(difference).lessThanOrEqualTo(2)
			) {
				return cost;
			} else {
	...
	getBuyAmount(noMaxBuy) {
		if (SharkGame.Settings.current.buyAmount === "custom") {
			return $("#custom-input")[0] &&
				Math.floor($("#custom-input")[0].valueAsNumber) >= 1 &&
				$("#custom-input")[0].valueAsNumber < 1e18
...
SharkGame.TextUtil = {
	...
	formatTime(milliseconds) {
		...
		const formatCentiseconds =
			(milliseconds / 1000 < 10
				? "." + numCentiseconds.toString(10).padStart(2, "0")
				: "") + (numMinutes === 0 ? "s" : "");
		...
		const formatMinutes =
			numMinutes > 0
				? (numMinutes % 60).toString(10).padStart(2, "0") + ":"
				: "";
		...
		const formatMonths =
			numMonths > 0 ? (numMonths % 12).toString() + "M, " : "";
	...
	getResourceName(
		...
		const amount =
			arbitraryAmount ||
			Math.floor(SharkGame.PlayerResources.get(resourceName).amount);
		let name =
			textToColor ||
			(amount - 1 < SharkGame.EPSILON ? resource.singleName : resource.name);
		let extraStyle = "";
		...
		if (SharkGame.Settings.current.colorCosts !== "none") {
			let color =
				SharkGame.Settings.current.colorCosts === "color"
					? resource.color
					: sharkcolor.getBrightColor(resource.color);
			if (darken) {
				...
				if (contrast < tolerance) {
					const requiredLuminance =
						tolerance * backRLum + 0.05 * tolerance - 0.05;
					color = sharkcolor.correctLuminance(
						color,
						requiredLuminance > 1
							? (backRLum + 0.05) / tolerance - 0.05
							: requiredLuminance,
					);
				}
			}
		...
		return (
			"<span class='click-passthrough'" + extraStyle + ">" + name + "</span>"
		);
	},
...
SharkGame.ColorUtil = {
	...
	correctLuminance(color, luminance) {
		...
		blue = blue / 255;
		const varA =
			1.075 * (0.2126 * red ** 2 + 0.7152 * green ** 2 + 0.0722 * blue ** 2);
		const varB = -0.075 * (0.2126 * red + 0.7152 * green + 0.0722 * blue);
		const ratio = Math.max(
			(-varB + Math.sqrt(varB ** 2 + 4 * varA * luminance)) / (2 * varA),
			0,
		);
		red = parseInt(Math.min(255, 255 * red * ratio, 10).toFixed(0), 10)
			.toString(16)
			.padStart(2, "0");
		green = parseInt(Math.min(255, 255 * green * ratio, 10).toFixed(0), 10)
			.toString(16)
			.padStart(2, "0");
		blue = parseInt(Math.min(255, 255 * blue * ratio, 10).toFixed(0), 10)
			.toString(16)
			.padStart(2, "0");
	...
	convertColorString(color) {
		...
		let colorstring = "#";
		for (let i = 0; i < 3; i++) {
			colorstring += ("00" + parseInt(colors[i * 2], 10).toString(16)).substr(
				parseInt(colors[i * 2], 10).toString(16).length,
			);
		}
		return colorstring;
	},
	...
	getBrightColor(color) {
		...
		red = parseInt((255 * (1 / most) * red).toFixed(0), 10).toString(16);
		green = parseInt((255 * (1 / most) * green).toFixed(0), 10).toString(16);
		blue = parseInt((255 * (1 / most) * blue).toFixed(0), 10).toString(16);
		return "#" + red + green + blue;
	},
					...
					for (const key of keys) {
						...
						if (descriptor.get !== undefined || descriptor.set !== undefined) {
							...
							switch (typeof descriptor.value) {
								...
								case "undefined":
									console.warn(
										"Property with value undefined? Huh?",
										descriptor,
										obj,
									);
									clone[key] = this.cloneDeep(descriptor.value);

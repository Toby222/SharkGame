SharkGame.Gate = {
				...
				$.each(gate.requirements.slots, (resource, requiredAmount) => {
					if (!gate.completedRequirements.slots[resource]) {
						...
						SharkGame.Button.makeHoverscriptButton(
							"gateCost-" + resource,
							"Insert " +
								sharktext.beautify(requiredAmount) +
								" " +
								resourceName +
								" into " +
								resourceName +
	...
	getUpgradesLeft() {
		...
		if (gate.requirements.upgrades) {
			return gate.requirements.upgrades.length === 0
				? incompleteUpgrades
				: false;
		} else {
			return false;
	...
	onGateButton() {
		...
		let message = "";
		const cost =
			gate.requirements.slots[resourceId] * (res.getResource("numen") + 1);
	...
	shouldBeOpen() {
		return _.every(SharkGame.Gate.completedRequirements, (requirementType) =>
			_.every(requirementType),
		);
	},
	...
	checkUpgradeRequirements(upgradeName) {
		...
		if (
			...
			gate.completedRequirements.upgrades[upgradeName] === false
		) {
	...
	checkResourceRequirements(resourceName) {
		...
		if (
			...
			res.getResource(resourceName) >= gate.requirements.resources[resourceName]
		) {

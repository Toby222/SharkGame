SharkGame.Lab = {
	...
	resetUpgrades() {
		...
		SharkGame.ResourceMap.forEach((_resource, resourceId) => {
			SharkGame.ModifierMap.get(resourceId).upgrade =
				_.cloneDeep(upgradeObject);
		});
	},
	...
	setHint(upgradeTable, isNotStart) {
		...
		if (lab.allResearchDone()) {
			...
			switch (world.worldType) {
				...
				default:
					message =
						"The scientists rest content, sure that they're done with their work.";
			}
			...
			switch (world.worldType) {
				case "tempestuous":
					if (res.getTotalResource("scientist") < 1) {
						...
						message =
							"The scientists are out of ideas, but there are always more discoveries to be made.";
					}
					break;
				case "volcanic":
					message =
						"The crabs are out of ideas, but there are always more discoveries to be made.";
					break;
				default:
					message =
						"The scientists are out of ideas, but there are always more discoveries to be made.";
			...
			const hintedUpgrade = _.find(
				...
				(upgrade, upgradeId) =>
					...
					_.every(upgrade.required.upgrades, (requiredUpgradeId) =>
			...
			if (_.has(hintedUpgrade, "required.seen")) {
				hintResource = _.find(hintedUpgrade.required.seen, (resource) =>
					world.doesResourceExist(resource),
				);
			} else if (_.has(hintedUpgrade, "required.totals")) {
				hintResource = _.find(
					Object.keys(hintedUpgrade.required.totals),
					(resource) => world.doesResourceExist(resource),
				);
			}
			...
			if (hintResource) {
					...
					$("<p>").html(
						"You get the feeling that " +
							sharktext.getResourceName(
								...
								false,
								2,
								sharkcolor.getElementColor("buttonList"),
							) +
							" may be the key.",
					),
				);
				...
				log.addError(
					`There is a possible, undiscovered upgrade (${hintedUpgrade.name}), but no valid hint resource.`,
				);
			}
		}
	},
			...
			if (button.length > 0) {
				...
				if (upgrade.required) {
					...
					if (upgrade.required.upgrades) {
						prereqsMet =
							prereqsMet && this.areRequiredUpgradePrereqsPurchased(upgradeId);
					}
	...
	areRequiredUpgradePrereqsPurchased(upgradeId) {
		const upgradeData = SharkGame.Upgrades.getUpgradeData(
			SharkGame.Upgrades.getUpgradeTable(),
			upgradeId,
		);

		if (upgradeData.required) {
			return _.every(upgradeData.required.upgrades, (requiredUpgradeId) =>
				SharkGame.Upgrades.purchased.includes(requiredUpgradeId),
			);
		}
		return true;
	...
	updateLabButton(upgradeName) {
		...
		const costText = sharktext.resourceListToString(
			upgradeCost,
			!enableButton,
			sharkcolor.getElementColor(upgradeName),
		);
		if (costText !== "") {
	...
	addUpgrade(upgradeId) {
		const upgrade = SharkGame.Upgrades.getUpgradeData(
			SharkGame.Upgrades.getUpgradeTable(),
			upgradeId,
		);
		if (upgrade && !SharkGame.Upgrades.purchased.includes(upgradeId)) {
			...
			const upgradeElt = $("<li>").html(
				`${upgrade.name}<br/><span class='medDesc'>${upgrade.effectDesc}</span>`,
			);
			...
			if (gotUpgradeTime) {
				console.log(
					`Added upgrade ${upgrade.name} at: ${sharktext.formatTime(gotUpgradeTime)}`,
				);
			} else {
				console.log(
					`Added upgrade ${upgrade.name} at: ${sharktext.formatTime(sharktime.getRunTime())}`,
				);
	...
	findAllAffordableUpgrades() {
		...
		$.each(table, (upgradeName) => {
			if (
				!this.isUpgradePossible(upgradeName) ||
				!this.isUpgradeVisible(upgradeName) ||
				SharkGame.Upgrades.purchased.includes(upgradeName)
			) {
				return true;
			}
			...
			if (
				res.checkResources(
					SharkGame.Upgrades.getUpgradeData(table, upgradeName).cost,
				) &&
		...
		if (upgradeData.required) {
			if (upgradeData.required.resources) {
				...
				_.each(upgradeData.required.resources, (resourceId) => {
					relatedResourcesExist =
						relatedResourcesExist || world.doesResourceExist(resourceId);
				});
			...
			isPossible =
				isPossible &&
				_.every(upgradeData.required.upgrades, (upgrade) =>
					lab.isUpgradePossible(upgrade),
				);

			// check resource cost
			isPossible =
				isPossible &&
				_.every(upgradeData.cost, (_amount, resource) =>
					world.doesResourceExist(resource),
				);
		}
	...
	isUpgradeVisible(upgradeId) {
		...
		if (_.has(upgrade, "required.seen")) {
			...
			isVisible =
				isVisible &&
				_.some(
					upgrade.required.seen,
					(requiredSeen) => res.getTotalResource(requiredSeen) > 0,
				);
		}
		if (_.has(upgrade, "required.totals")) {
			isVisible =
				...
				_.every(
					...
					(requiredTotal, resourceName) =>
						res.getTotalResource(resourceName) >= requiredTotal,
				);
		}
	...
	getResearchEffects(upgrade) {
		...
		const color = sharkcolor
			.getVariableColor("--color-light")
			.replace(/[^0-9a-f]/gi, "");
		...
		$.each(upgrade.effect, (effectType, effectsList) => {
			$.each(effectsList, (resource, degree) => {
				const effectText = SharkGame.ModifierReference.get(
					effectType,
				).effectDescription(degree, resource, darkerColour);
		...
		return (
			"<span class='medDesc' class='click-passthrough'>(Effects: " +

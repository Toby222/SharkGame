		...
		multiplier: {
			...
			incomeBoost: {
				...
				getEffect(genDegree, _outDegree, gen, out) {
					return SharkGame.ResourceMap.get(gen).income[out] > 0 && out !== "tar"
						? genDegree
						: 1;
				},
		...
		other: {
			addCoralIncome: {
				...
				apply(current, degree, resource) {
					...
					baseIncomes.coral =
						(baseIncomes.coral ? baseIncomes.coral : 0) + degree;
			...
			addJellyIncome: {
				...
				apply(current, degree, resource) {
					...
					const baseIncomes = SharkGame.ResourceMap.get(resource).baseIncome;
					baseIncomes.jellyfish =
						(baseIncomes.jellyfish ? baseIncomes.jellyfish : 0) + degree;
			...
			addSpongeIncome: {
				...
				apply(current, degree, resource) {
					...
					const baseIncomes = SharkGame.ResourceMap.get(resource).baseIncome;
					baseIncomes.sponge =
						(baseIncomes.sponge ? baseIncomes.sponge : 0) + degree;
			...
			addAlgaeIncome: {
				...
				apply(current, degree, resource) {
					...
					const baseIncomes = SharkGame.ResourceMap.get(resource).baseIncome;
					baseIncomes.algae =
						(baseIncomes.algae ? baseIncomes.algae : 0) + degree;
			...
			planetaryResourceBoost: {
				...
				apply(current, degree, boostedResource) {
					SharkGame.ResourceMap.forEach((generatingResource) => {
						$.each(generatingResource.income, (generatedResource, amount) => {
							if (generatedResource === boostedResource && amount > 0) {
								generatingResource.income[generatedResource] =
									amount * (1 + degree);
							}
						});
					});
		...
		other: {
			planetaryIncome: {
				...
				apply(current, degree, resource) {
					...
					baseIncomes[resource] =
						(baseIncomes[resource] ? baseIncomes[resource] : 0) + degree;
				...
				effectDescription(degree, resource, background) {
					return (
						"Gain " +
						sharktext.beautify(degree) +
						" " +
						sharktext.getResourceName(resource, false, degree, background) +
						" per second"
					);
				},
				applyToInput(input, _genDegree, _outDegree, _gen, _out) {
	...
	aspect: {
		multiplier: {
			...
			constructedConception: {
				...
				applyToInput(input, genDegree, _outDegree, _gen, out) {
					return (
						input * (input > 0 && out !== "tar" ? 2 ** (genDegree - 1) : 1)
					);
				},
			},

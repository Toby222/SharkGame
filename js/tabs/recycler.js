SharkGame.Recycler = {
	...
	message:
		"Convert things into residue, and residue into things!<br/><span class='medDesc'>Feed the machines. Feed them.</span>",
	...
	updateJunkDisplay() {
		...
		if (rec.expectedOutput !== "NA" && rec.expectedOutput !== 0) {
			junkString =
				...
				sharktext.beautify(junkAmount + rec.expectedOutput) +
				"</span> ";
		} else if (rec.expectedJunkSpent !== "NA" && rec.expectedJunkSpent !== 0) {
			junkString =
				"<span class='click-passthrough' style='color:#FFE436'>" +
				sharktext.beautify(junkAmount - rec.expectedJunkSpent) +
				"</span> ";
		} else {
	...
	updateButtons() {
		SharkGame.ResourceMap.forEach((_resource, resourceName) => {
			if (res.getTotalResource(resourceName) > 0) {
				...
				// update input button
				let disableButton =
					resourceAmount.lessThan(inputAmount) ||
					inputAmount.lessThanOrEqualTo(0);
				let label = "Recycle ";
				if (inputAmount.greaterThan(0)) {
					if (
						...
						!disableButton &&
						resourceName === rec.hoveredResource
					) {
						if (buy < 0) {
							label +=
								...
								sharktext.beautify(
									Number(
										inputAmount.plus(outputAmount).dividedBy(buy.times(-1)),
									),
								) +
								"</span> ";
						} else {
							label +=
								"<span class='click-passthrough' style='color:#FFDE0A'>" +
								sharktext.beautify(Number(inputAmount)) +
								"</span> ";
						}
					} else {
				...
				disableButton =
					maxOutputAmount.lessThan(outputAmount) ||
					outputAmount.lessThanOrEqualTo(0);
				label = "Convert to ";
				if (outputAmount > 0) {
					if (
						...
						label +=
							"<span class='click-passthrough' style='color:#FFDE0A'>" +
							sharktext.beautify(Number(outputAmount)) +
							"</span> ";
					} else {
				...
				SharkGame.Button.makeHoverscriptButton(
					...
					"Recycle " +
						sharktext.getResourceName(
							...
							undefined,
							undefined,
							sharkcolor.getVariableColor("--color-light"),
						),
					inputButtonDiv,
					rec.onInput,
					rec.onInputHover,
	...
	onInput() {
		...
		if (resourceAmount >= amount * (1 - SharkGame.EPSILON)) {
			res.changeResource(
				"junk",
				amount * junkPerResource * rec.getEfficiency(),
			);
			res.changeResource(resourceName, -amount);
			res.changeResource(
				"tar",
				Math.max(
					amount * junkPerResource * 0.0000002 +
						res.getProductAmountFromGeneratorResource("filter", "tar"),
					0,
				),
			);
			...
			log.addError(
				"Not enough resources for that transaction. This might be caused by putting in way too many resources at once.",
			);
		}
	...
	onOutput() {
		...
		const junkPerResource = new Decimal(
			SharkGame.ResourceMap.get(resourceName).value,
		);
		...
		if (costFunction === "linear") {
			junkNeeded = sharkmath.linearCost(
				currentResourceAmount,
				amount,
				junkPerResource,
			);
		} else if (costFunction === "constant") {
			junkNeeded = sharkmath.constantCost(
				currentResourceAmount,
				amount,
				junkPerResource,
			);
		...
		if (
			junkAmount.greaterThanOrEqualTo(junkNeeded.times(1 - SharkGame.EPSILON))
		) {
	...
	getMaxToBuy(resource) {
		...
		const junkPricePerResource = new Decimal(
		...
		if (rec.allowedCategories[category]) {
			...
			if (costFunction === "linear") {
				max = sharkmath
					.linearMax(resourceAmount, junkAmount, junkPricePerResource)
					.minus(resourceAmount);
			} else if (costFunction === "constant") {
				max = sharkmath
					.constantMax(resourceAmount, junkAmount, junkPricePerResource)
					.minus(resourceAmount);
			}
		}
		return max.round();
	...
	getTarString() {
		...
		if (world.worldType === "abandoned") {
			...
			const tarTolerance = -res.getProductAmountFromGeneratorResource(
				"filter",
				"tar",
			);
			let produced =
				SharkGame.ResourceMap.get(rec.hoveredResource).value * 0.0000002;
			if (buy > 0) {
				produced *= buy;
			} else {
			...
			amountstring =
				...
				sharktext.getResourceName(
					...
					undefined,
					undefined,
					sharkcolor.getElementColor("junkDisplay"),
				);
			if (tarTolerance > 0) {
				amountstring +=
					"<br/>(" +
	...
	getRecyclerEfficiencyString() {
		...
		if (sharkmath.getBuyAmount() > 0) {
			amountstring = sharktext.beautify(
				rec.efficiency * sharkmath.getBuyAmount(),
			);
		} else {
			amountstring = sharktext.beautify(
				(rec.efficiency * res.getResource(rec.hoveredResource)) /
					-sharkmath.getBuyAmount(),
			);
		}
	...
	updateExpectedOutput() {
		...
		if (buy > 0) {
			if (buy <= amount) {
				...
				rec.expectedOutput =
					buy * rec.getEfficiency() * SharkGame.ResourceMap.get(resource).value;
			}
		} else {
	...
	updateExpectedJunkSpent() {
		...
		if (buy > 0) {
			if (buy <= max) {
				switch (rec.allowedCategories[res.getCategoryOfResource(resource)]) {
					case "constant":
						...
						break;
					case "linear":
						rec.expectedJunkSpent = Number(
							sharkmath.linearCost(resourceAmount, buy, value),
						);
				}
			} else {
				rec.expectedJunkSpent = 0;
			...
			switch (rec.allowedCategories[res.getCategoryOfResource(resource)]) {
				...
				case "linear":
					rec.expectedJunkSpent = Number(
						sharkmath.linearCost(resourceAmount, realBuy, value),
					);
			}
		}
	...
	updateEfficiency(resource) {
		...
		if (purchaseAmount <= 10 ** maxEfficiencyRecyclePowerOfTen) {
			...
			rec.efficiency =
				1 /
				(Math.log10(purchaseAmount) -
					maxEfficiencyRecyclePowerOfTen +
					Math.round(1 / baseEfficiency));
		}
	},
};

SharkGame.CheatsAndDebug = {
	...
	cheatButtons: {
		...
		toggleFreeStuff: {
			get name() {
				return cad.actionPriceModifier
					? "Enable free stuff"
					: "Disable free stuff";
			},
			category: "modifiers",
		...
		toggleFreeUpgrades: {
			get name() {
				return cad.upgradePriceModifier
					? "Enable free upgrades"
					: "Disable free upgrades";
			},
			category: "modifiers",
			updates: true,
			click() {
			...
			switch (buttonData.type) {
				case "up-down":
					if (!buttonData.clickUp || !buttonData.clickDown) {
						log.addError(
							"Cheat button is up-down type, but has no functions for its buttons.",
						);
						return true;
					}
					container = $("<div>").attr("id", buttonName).addClass("up-down");
					container.append(
						"<span class='up-downText'>" + buttonData.name + "</span>",
					);
					...
					buttonContainer.append(
						$(
							"<button id='" +
								buttonName +
								"Up' class='min close-button'>⯅</button>",
						).on("click", buttonData.clickUp),
					);
					buttonContainer.append(
						$(
							"<button id='" +
								buttonName +
								"Down' class='min close-button'>⯆</button>",
						).on("click", buttonData.clickDown),
					);
	...
	cycleStyles(time = 2000) {
		...
		let intervalId = NaN;
		function nextStyle() {
			if (i >= gateway.allowedWorlds.length && !Number.isNaN(intervalId)) {
	...
	giveEverything(amount = 1) {
		...
		return (
			(amount > 0
				? "Gave " + sharktext.beautify(amount)
				: "Removed " + sharktext.beautify(-amount)) + " stuff."
		);
	},
			...
			case 14:
				if (world.doesResourceExist("crab")) {
					...
					if (world.worldType === "start") {
						...
						delete SharkGame.HomeActions.generated[world.worldType]
							.getCollector;
						delete SharkGame.HomeActions.generated[world.worldType]
							.getExtractionTeam;
					}
	...
	expensiveUpgradesPlease() {
		...
		switch (cad.upgradePriceModifier) {
			case 0:
				log.addError(
					"Can't change the price of upgrades because they're free.",
				);
				break;
			case 2:
			...
			default:
				msg =
					"Upgrades are " + cad.upgradePriceModifier + " times normal price.";
				break;
		}
		return msg;
	},
	...
	forceAllExist() {
		SharkGame.ResourceMap.forEach((resource, resourceId) => {
			if (resource.desc && resource.desc !== "")
				world.forceExistence(resourceId);
		});
		$("#content").empty();
		cad.switchTo();

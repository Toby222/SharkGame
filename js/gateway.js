SharkGame.Gateway = {
	...
	enterGate(loadingFromSave) {
		...
		if (!loadingFromSave) {
			...
			if (SharkGame.wonGame) {
				...
				SharkGame.persistentFlags.destinyRolls =
					SharkGame.Aspects.destinyGamble.level;
	...
	preparePlanetSelection(numPlanets) {
		...
		for (let i = 0; i < numPlanets; i++) {
			...
			if (uncompletedPlanetTypes.indexOf(choice) > -1) {
				uncompletedPlanetTypes.splice(
					uncompletedPlanetTypes.indexOf(choice),
					1,
				);
			}
	...
	playerHasSeenResource(resource) {
		...
		return _.some(gateway.completedWorlds, (completedWorld) =>
			_.some(
				SharkGame.WorldTypes[completedWorld].foresight.present,
				(seenResource) => seenResource === resource,
			),
		);
	},
	...
	getTimeInLastWorld(formatLess) {
		if (SharkGame.persistentFlags.lastRunTime) {
			return formatLess
				? SharkGame.persistentFlags.lastRunTime
				: sharktext.formatTime(SharkGame.persistentFlags.lastRunTime);
		} else {
	...
	updateWasScoutingStatus() {
		if (!_.isUndefined(SharkGame.persistentFlags.scouting)) {
			SharkGame.persistentFlags.wasScouting =
				SharkGame.persistentFlags.scouting;
	...
	updateScoutingStatus() {
		SharkGame.persistentFlags.scouting = !gateway.completedWorlds.includes(
			world.worldType,
		);
	},
	...
	currentlyOnScoutingMission() {
		if (
			!SharkGame.gameOver &&
			_.isUndefined(SharkGame.persistentFlags.scouting)
		) {
			gateway.updateScoutingStatus();
		}
			...
			if (SharkGame.persistentFlags.dialSetting > 1) {
				return (
					(SharkGame.Aspects.patience.level *
						2 *
						Math.log(SharkGame.persistentFlags.dialSetting)) /
					Math.log(4)
				);
			} else {
	...
	getGumptionBonus(loadingFromSave) {
		if (!loadingFromSave && SharkGame.wonGame) {
			const bonus =
				SharkGame.Aspects.gumption.level * 0.01 * res.getResource("essence");
	...
	grantEssenceReward(essenceReward, patienceReward, speedReward) {
		...
		res.changeResource(
			"essence",
			Math.ceil(
				(1 + gumptionBonus) * (essenceReward + speedReward) + patienceReward,
			),
		);
	},
	...
	shouldCheatsBeUnlocked() {
		return (
			res.getTotalResource("essence") >= 1000 &&
			!SharkGame.persistentFlags.unlockedDebug
		);
	},

	unlockCheats() {
		if (
			!SharkGame.persistentFlags.debug &&
			!SharkGame.persistentFlags.unlockedDebug
		) {
	...
	ui: {
		showGateway(
			...
			patienceReward,
			speedReward,
			gumptionRatio = gateway.getGumptionBonus(),
			forceWorldBased = false,
			storedTime = 0,
		) {
			const gumptionBonus = Math.ceil(
				gumptionRatio * (baseReward + speedReward),
			);
			...
			gatewayContent.append(
				$("<p>").html("You are a shark in the space between worlds."),
			);
			if (!SharkGame.wonGame) {
				gatewayContent.append(
					$("<p>")
			...
			gatewayContent.append(
				$("<p>")
					.html(sharktext.boldString("Something unseen says,"))
					.addClass("medDesc"),
			);
			gatewayContent.append(
				$("<em>")
					...
					.html(
						sharktext.boldString(
							gateway.getVoiceMessage(SharkGame.wonGame, forceWorldBased),
						),
					),
			);
			...
			if (speedReward > 0) {
				...
				SharkGame.wonGame &&
				!gateway.wasOnScoutingMission() &&
				!gateway.getMinutesBelowPar()
			) {
				gatewayContent.append(
					$("<p>").html(
						"You didn't beat this world fast enough to get below par. If you did, you would get more essence.",
					),
				);
			}
			if (gumptionBonus) {
				...
				gatewayContent.append(
					$("<p>").html(
						"You gained <span class='essenceCount'>" +
							sharktext.beautify(
								speedReward + patienceReward + baseReward + gumptionBonus,
								false,
								2,
							) +
			...
			gatewayContent.append(
				$("<p>").attr("id", "gatewayStatusMessage").addClass("medDesc"),
			);
				...
				() => {
					if (!SharkGame.Aspects.pathOfEnlightenment.level) {
						$("#tooltipbox")
							...
							.html("You're not yet sure what this means.");
					}
				},
				() => {
			...
			SharkGame.PaneHandler.swapCurrentPane(
				"GATEWAY",
				gatewayContent,
				true,
				500,
		...
		showRunEndInfo(containerDiv) {
			if (gateway.getTimeInLastWorld(true) < 0) {
				...
				containerDiv.append(
					$("<p>").html(
						`<em>Time spent within last ocean:</em><br/>${gateway.getTimeInLastWorld()}`,
					),
				);
			}
		},
		...
		showAspects() {
			...
			aspectTreeContent.append(
				$("<strong>")
					...
					.attr(
						"contenteditable",
						SharkGame.persistentFlags.debug ? "true" : "false",
					)
			...
			aspectTreeContent.append(
				tree.drawTree(SharkGame.Settings.current.doAspectTable === "table"),
			);

			const buttonDiv = $("<div>")
				.attr("id", "aspectTreeNavButtons")
				.addClass("gatewayButtonList");
			...
			SharkGame.Button.makeButton(
				...
				() => {
					...
					$("#tooltipbox")
						.empty()
						.removeClass("forAspectTree forAspectTreeUnpurchased");
				},
			);
		...
		showPlanets(foregoAnimation) {
			...
			if (SharkGame.Aspects.destinyGamble.level > 0) {
				SharkGame.Button.makeButton(
					"destinyGamble",
					"foobar",
					planetSelectionContent,
					gateway.rerollWorlds,
				);
			}
		...
		formatDestinyGamble() {
			if (!_.isUndefined(SharkGame.persistentFlags.destinyRolls)) {
				switch (SharkGame.persistentFlags.destinyRolls) {
					case 0:
						$("#destinyGamble")
							.html("No rerolls remain. Beat a world to recharge.")
							.addClass("disabled");
						break;
					case 1:
		...
		confirmWorld() {
			...
			const seenWorldYet = gateway.completedWorlds.includes(
				gateway.selectedWorld,
			);
			...
			const gatewayContent = $("<div>").append(
				$("<p>").html(
					(seenWorldYet
						? "Replay the " + selectedWorldData.name + " W"
						: "Scout this w") + "orld?",
				),
			);
							...
							sharktext.beautify(
								Math.ceil(
									(1 + gateway.getGumptionBonus()) *
										((seenWorldYet ? 2 : 4) +
											(selectedWorldData.bonus ? selectedWorldData.bonus : 0)) +
										SharkGame.Aspects.patience.level *
			...
			// add world image
			const spritename = seenWorldYet
				? "planets/" + gateway.selectedWorld
				: "planets/missing";
			...
			gateway.ui.showPlanetAttributes(
				selectedWorldData,
				seenWorldYet,
				attributeDiv,
			);
			...
			if (seenWorldYet && selectedWorldData.par) {
				gatewayContent.append(
					$("<p>").html(
						"Par: <strong>" +
							selectedWorldData.par +
							" minutes</strong><br>Beat the world faster for extra essence.",
					),
				);
			}
			...
			if (SharkGame.Aspects.theDial.level) {
				...
				const dial = $("<input>")
					...
					.attr(
						"value",
						Math.round(
							Math.log(SharkGame.persistentFlags.dialSetting) / Math.log(4),
						) + 1,
					)
				...
				if (SharkGame.persistentFlags.dialSetting > 1) {
					...
					dialLabel = $("<p>")
						.attr("id", "dial-label")
						.html(
							"Adjust The Dial to modify Patience rewards.<br>...or don't. If you don't want to.",
						);
				}
			...
			SharkGame.Button.makeButton(
				...
				() => {
					...
					if (
						gateway.completedWorlds.includes(gateway.selectedWorld) ||
						checkAspects()
					) {
		...
		showPlanetAttributes(worldData, seenWorldYet, contentDiv) {
			switch (SharkGame.Aspects.distantForesight.level) {
				case 1:
					...
					if (
						worldData.foresight.missing &&
						worldData.foresight.missing.length > 0
					) {
					...
					if (
						worldData.foresight.present &&
						worldData.foresight.present.length > 0
					) {
									...
									"You feel the presence of " +
										sharktext.getResourceName(
											...
											gateway.playerHasSeenResource(presentResource)
												? undefined
												: gateway.PresenceFeelings[presentResource],
										) +
										".",
								),
							);
					...
					if (worldData.modifiers && worldData.modifiers.length > 0) {
						...
						_.each(worldData.modifiers, (modifier) => {
							if (
								gateway.playerHasSeenResource(modifier.resource) ||
								!(worldData.foresight.present.indexOf(modifier.resource) > -1)
							) {
								modifierList.append(
									$("<li>").html(
										SharkGame.ModifierReference.get(
											modifier.modifier,
										).effectDescription(
											modifier.amount,
											modifier.resource,
								...
								modifierList.append(
									$("<li>").html(
										SharkGame.ModifierReference.get(modifier.modifier)
											...
											.replace(
												new RegExp(modifier.resource, "g"),
												gateway.PresenceFeelings[modifier.resource],
											),
									),
								);
							}
				...
				default:
					if (seenWorldYet) {
						...
						contentDiv.prepend(
							$("<p>").html(worldData.foresight.vagueLongDesc),
						);
					}
			}
		},
				...
				if (buttonSel.length > 0) {
					...
					const label =
						...
						(seenWorldYet
							...
							: deeperPlanetData.vagueDesc) +
						(seenWorldYet && gateway.getPar(planetData.type)
							? "<br>Par: <strong>" +
								gateway.getPar(planetData.type) +
								" minutes</strong>"
							: "");
		...
		showMinuteHandStorageExtraction(worldtype) {
			function getRequestedTime() {
				...
				if (
					!$.isEmptyObject($("#storage-years")) &&
					$("#storage-years")[0] &&
					$("#storage-years")[0].value
				) {
					years = Number($("#storage-years")[0].value);
				}
				if (
					!$.isEmptyObject($("#storage-months")) &&
					$("#storage-months")[0] &&
					$("#storage-months")[0].value
				) {
					months = Number($("#storage-months")[0].value);
				}
				if (
					!$.isEmptyObject($("#storage-weeks")) &&
					$("#storage-weeks")[0] &&
					$("#storage-weeks")[0].value
				) {
					weeks = Number($("#storage-weeks")[0].value);
				}
				if (
					!$.isEmptyObject($("#storage-days")) &&
					$("#storage-days")[0] &&
					$("#storage-days")[0].value
				) {
					days = Number($("#storage-days")[0].value);
				}
				if (
					!$.isEmptyObject($("#storage-hours")) &&
					$("#storage-hours")[0] &&
					$("#storage-hours")[0].value
				) {
					hours = Number($("#storage-hours")[0].value);
				}
				if (
					!$.isEmptyObject($("#storage-minutes")) &&
					$("#storage-minutes")[0] &&
					$("#storage-minutes")[0].value
				) {
					minutes = Number($("#storage-minutes")[0].value);
				}
				if (
					!$.isEmptyObject($("#storage-seconds")) &&
					$("#storage-seconds")[0] &&
					$("#storage-seconds")[0].value
				) {
			...
			function updateRequestedTime() {
				...
				if (
					...
					!gateway.completedWorlds.includes(worldtype)
				) {
					requestedTime = 600000;
				}
				...
				$("#requested-time").html(
					`You would take ${sharktext.boldString(res.minuteHand.formatMinuteTime(requestedTime, true))} with you.`,
				);
			}

			const menuContent = $("<div>").append(
				$("<p>").html(
					`You have some ${SharkGame.Settings.current.idleEnabled ? "idle " : ""}time in storage.`,
				),
			);
			const timeSelection = $("<div>").attr("id", "minute-storage-selection");

			const timeLeft = res.minuteHand.formatMinuteTime(
				SharkGame.persistentFlags.minuteStorage,
				true,
			);
			timeSelection.append(
				$("<p>").html(`There is ${sharktext.boldString(timeLeft)} left.`),
			);
			...
			if (!gateway.completedWorlds.includes(worldtype)) {
				timeSelection.append(
					$("<p>").html(
						sharktext.boldString(
							"Since you're going on a scouting mission, you can take up to 10 minutes with you.",
						),
					),
				);
			}
			...
			timeSelection.append(
				$("<p>").html(
					"How much would you like to take with you to the next world?",
				),
			);
			...
			if (gateway.completedWorlds.includes(worldtype)) {
				switch (precision) {
					// biome-ignore lint/suspicious/noFallthroughSwitchClause: Explicit fallthrough
					case 7:
						timeSelection.append(
							$("<input>")
			...
			timeSelection.append(
				$("<p>").html(
					sharktext.boldString(
						"ONLY TAKE AS MUCH AS YOU NEED!<br>Anything that you take but don't use will be discarded.",
					),
				),
			);
		...
		changeSetting(_event, arbitrary) {
			if (arbitrary) {
				...
				SharkGame.persistentFlags.dialSetting = Math.round(
					4 ** (document.getElementById("dial-slider").value - 1),
				);
			}
		...
		updateVisuals() {
			if (SharkGame.persistentFlags.dialSetting > 1) {
                ...
                Patience rewards ×${
									SharkGame.persistentFlags.dialSetting > 1
										? Math.round(
												(2 * Math.log(SharkGame.persistentFlags.dialSetting)) /
													Math.log(4),
											)
										: 1
								}`),

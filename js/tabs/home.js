SharkGame.Home = {
	...
	switchTo() {
		...
		if (!SharkGame.flags.selectedHomeMessage) {
			SharkGame.flags.selectedHomeMessage =
				SharkGame.HomeMessages.messages[world.worldType][0].name;
		}
		...
		$.each(SharkGame.HomeActionCategories, (categoryName, category) => {
			...
			if (categoryName === "all") {
				...
				categoryDiscovered = _.some(category.actions, (actionName) => {
					...
					return (
						_.has(actionTable, actionName) && actionTable[actionName].discovered
					);
				});
			}
	...
	getButtonTabs() {
		...
		$.each(SharkGame.HomeActionCategories, (categoryName) => {
			if (
				$(`#buttonTab-${categoryName}`).html() ||
				home.currentButtonTab === categoryName
			) {
	...
	tickHomeMessages(isDuringTabSwitch) {
		...
		const currentMessageData =
			SharkGame.HomeMessages.messages[world.worldType][
				SharkGame.Memories.messageLookup.get(
					SharkGame.flags.selectedHomeMessage,
				)
			];
		...
		if (
			anyUnseen ||
			((lastValidMessage.transient || currentMessageData.transient) &&
				currentMessageData.name !== lastValidMessage.name)
		) {
	...
	getLastValidMessage() {
		const lastValidMessageData = _.findLast(
			SharkGame.HomeMessages.messages[world.worldType],
			(extraMessage) => {
				...
				if (_.has(extraMessage, "unlock")) {
					...
					requirementsMet =
						requirementsMet &&
						_.every(extraMessage.unlock.upgrade, (upgradeId) =>
							SharkGame.Upgrades.purchased.includes(upgradeId),
						);
					requirementsMet =
						requirementsMet &&
						_.every(extraMessage.unlock.homeAction, (actionName) => {
							const action = SharkGame.HomeActions.getActionData(
	...
	findNextHomeMessage() {
		const currentIndex = SharkGame.Memories.messageLookup.get(
			SharkGame.flags.selectedHomeMessage,
		);
	...
	updateMessage(requestedMessage, suppressAnimation) {
		...
		const messageData =
			homeMessages[SharkGame.Memories.messageLookup.get(requestedMessage)];
		...
		if (tabMessage[0].children.length === 0) {
			...
			let message =
				"<strong class='medDesc'>You are a shark in a " +
				worldType.shortDesc +
				" sea.</strong>";
	...
	updateMessageSelectors() {
		...
		if (
			!home.findPreviousHomeMessage() ||
			home.getLastValidMessage().transient
		) {
		...
		if (!home.getLastValidMessage().transient) {
			_.each(
				...
				(messageData) => {
					...
					if (
						SharkGame.flags.seenHomeMessages.includes(messageData.name) &&
						!messageData.transient
					) {
						tracker.append(
							$("<div>")
								.html("•")
								.addClass(
									messageData.name === SharkGame.flags.selectedHomeMessage
										? ""
										: "inactive",
								),
						);
	...
	update() {
		...
		$.each(SharkGame.HomeActions.getActionTable(), (actionName, actionData) => {
			...
			const onTab =
				actionTab === home.currentButtonTab || home.currentButtonTab === "all";
		...
		if (
			document
				...
				.className.split(" ")
				.includes("forHomeButtonOrGrotto")
		) {
	...
	updateButton(actionName) {
		...
		const button = $("#" + actionName);
		const actionData = SharkGame.HomeActions.getActionData(
			SharkGame.HomeActions.getActionTable(),
			actionName,
		);
		...
		if (amountToBuy.lessThan(0)) {
			...
			if (amount.lessThan(1)) {
				if (
					amount
						.times(amountToBuy.times(-1))
						.greaterThanOrEqualTo(1 - SharkGame.EPSILON)
				) {
					enableButton = true;
				} else {
					enableButton = false;
		...
		if (_.some(actionCost, (cost) => !cost.isFinite())) {
			...
			const costText = sharktext.resourceListToString(
				actionCost,
				!enableButton,
				sharkcolor.getElementColor(actionName, "background-color"),
			);
			if (costText !== "") {
		...
		if (
			button.html().includes("button-icon") !==
			SharkGame.Settings.current.showIcons
		) {
			button.html(label);
			...
			switch (actionName) {
				case "getUrchin":
					...
					spritename =
						Math.random() < 0 ? "actions/getUrchinHatted" : "actions/getUrchin";
					break;
				case "getLobster":
					spritename =
						Math.random() < 0.01 ? "actions/getLobter" : "actions/getLobster";
					break;
				case "getSpawner":
					spritename =
						Math.random() < 0.01
	...
	areActionPrereqsMet(actionName) {
		...
		if (
			action.prereq.resource &&
			!res.checkResources(action.prereq.resource, true)
		) {
			return false;
		}
		...
		if (
			!_.every(action.cost, (cost) => world.doesResourceExist(cost.resource))
		) {
			return false;
		}
		...
		if (action.prereq.notWorlds?.includes(world.worldType)) {
			return false;
		}

		// check upgrade prerequisites
		if (
			!_.every(action.prereq.upgrade, (upgradeId) =>
				SharkGame.Upgrades.purchased.includes(upgradeId),
			)
		) {
			return false;
		...
		if (
			!_.every(action.effect.resource, (_amount, resourceId) =>
				...
				case "totalResourceThreshold":
					disable =
						...
						_.some(
							when,
							(resourceObject) =>
								res.getTotalResource(resourceObject.resource) >=
								resourceObject.threshold,
						);
					break;
				case "otherActions":
					disable =
						disable ||
						_.some(when, (otherAction) =>
							home.areActionPrereqsMet(otherAction),
						);
					break;
				case "upgrades":
					disable =
						disable ||
						_.some(when, (upgrade) =>
							SharkGame.Upgrades.purchased.includes(upgrade),
						);
					break;
				case "custom":
	...
	addButton(actionName) {
		...
		buttonSelector.html(
			$("<span id='" + actionName + "Label' class='click-passthrough'></span>"),
		);
		home.updateButton(actionName);
			...
			if (
				_.some(
					...
					(incomeAmount, resource) =>
						...
						((incomeAmount < 0 && !res.isInCategory(resource, "harmful")) ||
							(res.isInCategory(resource, "harmful") && incomeAmount > 0)),
				)
			) {
				givesBadThing = true;
				return false;
	...
	onHomeButton(_placeholder, actionName) {
		...
		if ($.isEmptyObject(actionCost) && !amount.equals(0)) {
			...
			if (res.checkResources(actionCost)) {
				...
				if (action.effect.resource) {
					let resourceChange;
					if (!amount.equals(1)) {
						resourceChange = res.scaleResourceList(
		...
		if (button.hasClass("newlyDiscovered")) {
			SharkGame.HomeActions.getActionTable()[actionName].newlyDiscovered =
				false;
	...
	onHomeHover(mouseEnterEvent, actionName) {
		if (
			!SharkGame.Settings.current.showTooltips ||
			(!actionName && !mouseEnterEvent) ||
			!main.shouldShowTooltips()
		) {
			return;
		}
		...
		$.each(effects.resource, (resource) => {
			$.each(SharkGame.ResourceMap.get(resource).income, (incomeResource) => {
				const genAmount = res.getProductAmountFromGeneratorResource(
					resource,
					incomeResource,
					1,
				);
		...
		if (
			...
			buyingHowMuch = sharkmath.getPurchaseAmount(
				undefined,
				home.getMax(actionData).toNumber(),
			);
			if (buyingHowMuch < 1) {
				buyingHowMuch = 1;
			}
		...
		const usePlural =
			(_.some(effects.resource, (_amount, name) =>
				sharktext.getDeterminer(name),
			) &&
				(buyingHowMuch > 1 ||
					_.some(effects.resource, (amount) => amount > 1))) ||
		...
		if (_.some(validGenerators, (amount) => amount > 0)) {
			if (
				_.some(
					validGenerators,
					(amount, resourceName) =>
						amount > 0 && res.isInCategory(resourceName, "harmful"),
				)
			) {
				$("#tooltipbox").addClass("gives-consumer");
			}
			text +=
				"<span class='littleTooltipText'>PRODUCE" +
				(usePlural ? "" : "S") +
				"</span><br/>";
		...
		if (!$.isEmptyObject(condensedObject.resAffect.increase)) {
			if (
				_.some(validGenerators, (_degree, resourceName) =>
					res.isInCategory(resourceName, "harmful"),
				)
			) {
				$("#tooltipbox").addClass("gives-consumer");
			}
			...
			addedAnyLabelsYet = true;
			$.each(
				condensedObject.resAffect.increase,
				(affectedResource, degreePerPurchase) => {
					text +=
						...
						sharktext.boldString(
							sharktext.beautify(buyingHowMuch * degreePerPurchase * 100) + "%",
						) +
						"<br>";
				},
			);
		...
		if (!$.isEmptyObject(condensedObject.resAffect.decrease)) {
			if (
				_.some(
					condensedObject.resAffect.decrease,
					(_degree, resourceName) => !res.isInCategory(resourceName, "harmful"),
				)
			) {
				$("#tooltipbox").addClass("gives-consumer");
			}
		...
		if (!$.isEmptyObject(condensedObject.resAffect.multincrease)) {
			if (
				_.some(
					condensedObject.resAffect.multincrease,
					(_degree, resourceName) => res.isInCategory(resourceName, "harmful"),
				)
			) {
				$("#tooltipbox").addClass("gives-consumer");
			...
			$.each(
				...
				(affectedResource, degreePerPurchase) => {
					...
					text +=
						...
						" by " +
						sharktext.boldString(
							sharkmath.beautif(degreePerPurchase * 100) + "%",
						) +
						"<br>";
				},
			);
		...
		if (!$.isEmptyObject(condensedObject.resAffect.multdecrease)) {
			if (
				_.some(
					condensedObject.resAffect.multdecrease,
					(_degree, resourceName) => !res.isInCategory(resourceName, "harmful"),
				)
			) {
				$("#tooltipbox").addClass("gives-consumer");
			...
			$.each(
				...
				(affectedResource, degreePerPurchase) => {
					...
					text +=
						...
						" by " +
						sharktext.boldString(
							sharktext.beautify(degreePerPurchase * 100) + "%",
						) +
						"<br>";
				},
			);
		...
		if (!$.isEmptyObject(condensedObject.genAffect.increase)) {
			...
			addedAnyLabelsYet = true;
			$.each(
				condensedObject.genAffect.increase,
				(affectedGenerator, degreePerPurchase) => {
					text +=
						sharktext.getResourceName(
							affectedGenerator,
							false,
		...
		if (!$.isEmptyObject(condensedObject.genAffect.decrease)) {
			...
			addedAnyLabelsYet = true;
			$.each(
				condensedObject.genAffect.decrease,
				(affectedGenerator, degreePerPurchase) => {
					text +=
						sharktext.getResourceName(
							affectedGenerator,
		...
		if (!$.isEmptyObject(condensedObject.genAffect.multincrease)) {
			...
			addedAnyLabelsYet = true;
			$.each(
				condensedObject.genAffect.multincrease,
				(affectedGenerator, degreePerPurchase) => {
					degreePerPurchase = degreePerPurchase ** buyingHowMuch - 1;
					text +=
						sharktext.getResourceName(
		...
		if (!$.isEmptyObject(condensedObject.genAffect.multdecrease)) {
			...
			addedAnyLabelsYet = true;
			$.each(
				condensedObject.genAffect.multdecrease,
				(affectedGenerator, degreePerPurchase) => {
					degreePerPurchase = 1 - degreePerPurchase ** buyingHowMuch;
					text +=
						sharktext.getResourceName(
		...
		if (SharkGame.HomeActions.getActionTable()[actionName].helpText) {
			text +=
				...
				SharkGame.HomeActions.getActionTable()[actionName].helpText +
				"</span>";
		}
			...
			if (buyingHowMuch * amount !== 1) {
				text =
					...
					(SharkGame.Settings.current.tooltipQuantityReminders
						? "<span class='medDesc littleTooltipText'>(you have " +
							sharktext.beautify(res.getResource(resource)) +
							")</span><br>"
						: "") +
					text;
			} else {
		...
		if (
			...
			text.replace(/'/g, '"').replace(/br\//g, "br")
		) {
	...
	getMax(action) {
		...
		if (action.max) {
			...
			_.each(action.cost, (costObject) => {
				const costResource = new Decimal(
					SharkGame.PlayerResources.get(costObject.resource).amount,
				);
				...
				switch (costObject.costFunction) {
					case "constant":
						subMax = sharkmath.constantMax(
							typeof costResource === "object" ? new Decimal(0) : 0,
							costResource,
							priceIncrease,
						);
						break;
					case "linear":
						subMax = sharkmath
							.linearMax(currAmount, costResource, priceIncrease)
							.minus(currAmount);
						break;
					case "unique":
						subMax = sharkmath
							.uniqueMax(currAmount, costResource, priceIncrease)
							.minus(currAmount);
						break;
				}

...
SharkGame.AspectTree = {
	...
	staticButtons: {
		...
		respecAll: {
			...
			get posY() {
				return (
					tree.staticButtons.respec.posY + tree.staticButtons.respec.height + 10
				);
			},
			width: 30,
			height: 30,
			...
			clicked() {
				if (
					confirm("Are you sure you want to respec ALL refundable aspects?")
				) {
					tree.respecTree();
				}
			},
	...
	setup() {
		...
		if (SharkGame.persistentFlags.patience) {
			if (SharkGame.Aspects.patience.level) {
				res.changeResource(
					"essence",
					2 * (SharkGame.Aspects.patience.level + 1) ** 2,
				);
			}
	...
	drawTable(table = $("<table>")) {
		...
		$.each(SharkGame.Aspects, (aspectId, aspectData) => {
			...
			if (!reqref.prereqsMet && aspectData.level === 0) {
				cantBuyText =
					"With your infinite vision, you can see this aspect, but cannot buy it.";
			} else if (reqref.locked) {
				cantBuyText = "This aspect is locked. " + reqref.locked;
			} else if (reqref.isolated) {
				cantBuyText =
					"This aspect's prerequisites aren't met, even though you have levels in it.";
			}
			basicText =
				" A" +
			...
			const aspectNameTableData = $("<th>")
				.html(aspectData.name)
				.addClass("aspectTableName")
				.attr("rowspan", "3")
				.attr("scope", "rowgroup");
			const specialData = $("<td>").append(basicText + cantBuyText);
			const aspectTableDescriptionRow = $("<tr>")
				.append(aspectNameTableData)
				.append(specialData);
			...
			if (aspectData.level > 0) {
				aspectTableRowCurrent.append(
					$("<td>").html(`CURRENT: ${aspectData.getEffect(aspectData.level)}`),
				);
			} else {
				aspectTableRowCurrent.append(
					$("<td>").html("CURRENT: Not bought, no effect."),
				);
			}
			...
			if (!reqref.max) {
				...
				aspectTableRowNext.append(
					$("<td>").html(`NEXT: ${aspectData.getEffect(aspectData.level + 1)}`),
				);
				aspectTableRowNext.append($("<td>").html(`${aspectData.level + 1}`));
			} else {
				aspectTableRowNext.append(
					$("<td>").html("NEXT: Already at maximum level."),
				);
	...
	drawCanvas() {
		...
		$(canvas).on(
			"mouseenter mousemove mouseleave wheel click touchstart touchmove touchend touchcancel",
			tree.updateMouse,
		);
		$(canvas).on("click", tree.click);

		tree.context = canvas.getContext("2d", {
			alpha: false,
			desynchronized: true,
		});
	...
	initTree() {
		this.panzoom = panzoom($("canvas")[0], {
			...
			beforeMouseDown: (event) =>
				event.target.id !== "treeCanvas" ||
				this.getButtonUnderMouse(event) !== undefined,
	...
	getButtonUnderMouse(event) {
		...
		const staticButton = _.find(
			tree.staticButtons,
			({ posX, posY, width, height }) => {
				return (
					mousePos.posX - posX >= 0 &&
					mousePos.posY - posY >= 0 &&
					mousePos.posX - posX <= width &&
					mousePos.posY - posY <= height
				);
			},
		);
		if (
			staticButton !== undefined &&
			(!staticButton.getUnlocked || staticButton.getUnlocked())
		) {
			return staticButton;
		}
		...
		const aspect = _.find(
			SharkGame.Aspects,
			({ posX, posY, width, height, prerequisites, level }) => {
				if (
					_.some(
						prerequisites,
						(prerequisite) => SharkGame.Aspects[prerequisite].level === 0,
					) &&
				...
				return (
					computedX >= posX &&
					computedY >= posY &&
					computedX <= posX + width &&
					computedY <= posY + height
				);
			},
		);
		return aspect;
	...
	updateMouse(event) {
		const button =
			event.type === "mouseleave" ? undefined : tree.getButtonUnderMouse(event);
	...
	click(event) {
		...
		const isMouseClick =
			...
			event.originalEvent.mozInputSource === 1 ||
			event.originalEvent.mozInputSource === 2;
		if (
			typeof button.clicked === "function" &&
			(isMouseClick || tree.previousButton?.name === button?.name)
		) {
	...
	render() {
		...
		const buttonStyle = getComputedStyle(
			document.getElementById("backToGateway"),
		);
		...
		_.each(
			SharkGame.Aspects,
			({ level, posX, posY, width, height, requiredBy, deprecated }) => {
		...
		_.each(
			SharkGame.Aspects,
			(
				{ posX, posY, width, height, icon, eventSprite, level, deprecated },
				name,
			) => {
				if (deprecated) return;
		...
		_.each(
			tree.staticButtons,
			({ posX, posY, width, height, icon, eventSprite }, name) => {
	...
	renderButton(
		...
		context.quadraticCurveTo(
			...
			posY + height,
			posX,
			posY + height - BUTTON_BORDER_RADIUS,
		);
		...
		if (icon !== null) {
			if (
				icon === "general/missing-action" &&
				SharkGame.Sprites["aspects/" + name]
			) {
				icon = "aspects/" + name;
			}
	...
	getLittleLevelText(aspectName) {
		...
		if (!reqref.locked && reqref.prereqsMet) {
			if (!reqref.max) {
				return (
					SharkGame.Aspects[aspectName].level +
					" / " +
					SharkGame.Aspects[aspectName].max
				);
			}
			return "MAX";
		}
		...
		if (!ignoreRestrictions) {
			if (
				...
				_.some(
					...
					(prereq) => SharkGame.Aspects[prereq].level === 0,
				)
			) {
				return;
			}
	...
	updateEssenceCounter() {
		if (document.getElementById("essenceCount")) {
			document.getElementById("essenceCount").innerHTML = sharktext.beautify(
				res.getResource("essence"),
				false,
				2,
			);
		}
	...
	resetScoutingRestrictions() {
		...
		$.each(SharkGame.Aspects, (aspectName) => {
			if (!_.isUndefined(SharkGame.persistentFlags.aspectStorage[aspectName])) {
				SharkGame.Aspects[aspectName].level =
					SharkGame.persistentFlags.aspectStorage[aspectName];
	...
	updateTooltip(button) {
		...
		if (button === undefined) {
			...
			tooltipBox
				.empty()
				.removeClass(
					"forAspectTree forAspectTreeUnpurchased forAspectTreeAffordable",
				);
		} else {
			let name;
			...
			if (button.level === 0) {
				...
				const levelText =
					(button.core ? " core aspect" : "") +
					(button.core && button.noRefunds ? ", " : "") +
	...
	updateRequirementReference() {
		...
		_.forEach(SharkGame.Aspects, (aspectData, aspectName) => {
			if (aspectData.deprecated) return;
			reqref[aspectName].affordable =
				aspectData.getCost(aspectData.level) <= res.getResource("essence");
			reqref[aspectName].locked = aspectData.getUnlocked() || false;
			reqref[aspectName].prereqsMet = !_.some(
				aspectData.prerequisites,
				(prereq) => SharkGame.Aspects[prereq].level === 0,
			);
			reqref[aspectName].isolated =
				aspectData.level && !reqref[aspectName].prereqsMet;
			reqref[aspectName].revealed =
				reqref[aspectName].prereqsMet ||
				SharkGame.Aspects.infinityVision.level ||
				aspectData.level;

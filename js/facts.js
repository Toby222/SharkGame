SharkGame.FunFacts = {
	...
	getPool() {
		...
		if (
			...
			(!this.worldBased[currentWorld].areRequirementsMet ||
				this.worldBased[currentWorld].areRequirementsMet())
		) {
		...
		$.each(this.resourceBased, (resource, facts) => {
			...
			if (world.doesResourceExist(resource) && res.getTotalResource(resource)) {
				...
				if (
					!this.dilutedResources.includes(resource) ||
					currentWorld === "start" ||
					Math.random() < 0.25
				) {
					_.each(facts, (fact) => {
		...
		if (anyAvailableResource) {
			...
			return [
				"Fun fact: <i>New fun facts are unlocked as you see new stuff. Keep playing to unlock some!</i>",
			];
		}
	},
	...
	worldBased: {
		...
		tempestuous: {
			messages: [
				"'Tempestuous' does not mean stormy. It means emotionally turbulent. But it's close enough.",
			],
		},
	},
	...
	resourceBased: {
		...
		kelp: [
			"Kelp is not a plant, it's a kind of algae. Algae is also not a plant.",
		],
		seagrass: [
		...
		sacrifice: [
			"Nobody knows how the energy of the crystals gets into the frenzy.",
		],
		...
		investigator: [
			"We are not sure where the octopuses get their funny hats. Presumably they just find them.",
		],
		...
		ice: [
			"In the original shark game, ice used to eat away your resources instead of slowing their production.",
		],
		tar: [
			"In the original shark game, tar was gained passively. Machines produced basically none of it.",
		],
		calciniumConverter: [

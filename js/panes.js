...
SharkGame.PaneHandler = {
	...
	showPane(
		...
		const overlayOpacity = $("#overlay").hasClass("gateway")
			? 1.0
			: customOpacity || 0.5;
		...
		if (!notCloseable) {
			document
				...
				.addEventListener("click", SharkGame.PaneHandler.nextPaneInStack);
			overlay.addClass("pointy");
		} else {
			document
				.getElementById("overlay")
				.removeEventListener("click", SharkGame.PaneHandler.nextPaneInStack);
	...
	setUpOptions() {
		...
		$.each(categories, (category, settings) => {
			optionsTable.append(
				$("<tr>").html(
					"<h3><br><span style='text-decoration: underline'>" +
						sharktext.boldString(category) +
						"</span></h3>",
				),
			);
					...
					optionRow.append(
						$("<td>").append(
							$("<button>")
								...
								.addClass(
									"option-button" + (isSelectedOption ? " disabled" : ""),
								)
								.html(
		...
		row.append(
			$("<td>").html(
				"Save Backups:<br/><span class='smallDesc'>(Create a backup save.)</span>",
			),
		);
		row2.append(
			$("<td>").html(
				"Load Backups:<br/><span class='smallDesc'>(Load a backup save.)</span>",
			),
		);
		...
		_.each(["1", "2", "3"], (tag) => {
					...
					$("<button>")
						...
						.on("click", () => {
							if (SharkGame.Save.savedGameExists(`Backup${tag}`)) {
								if (
									!confirm(
										"There is already a save in this slot. Overwrite it?",
									)
								) {
									return;
								}
			...
			const loadButton = $("<button>")
				...
				.on("click", () => {
					if (
						!$(`#load${tag}`).hasClass("disabled") &&
						SharkGame.Save.savedGameExists(`Backup${tag}`)
					) {
						if (
							confirm(
		...
		if (SharkGame.persistentFlags.unlockedDebug) {
			const loadButton = $("<button>")
				...
				.on("click", () => {
					if (
						!$("#loadCheats").hasClass("disabled") &&
						SharkGame.Save.savedGameExists("BackupCheats")
					) {
						if (
							confirm(
		...
		row.append(
			$("<td>").html(
				"Wipe Settings:<br/><span class='smallDesc'>(Change all settings to default.)</span>",
			),
		);
		row.append(
			$("<td>").append(
				$("<button>")
					...
					.on("click", () => {
						if (
							confirm(
								"Are you absolutely sure you want to wipe your settings to default?",
							)
						) {
							$.each(SharkGame.Settings.current, (settingName) => {
								if (SharkGame.Settings[settingName]) {
									SharkGame.Settings.current[settingName] =
										SharkGame.Settings[settingName].defaultSetting;
									if (
										typeof SharkGame.Settings[settingName].onChange ===
										"function"
									) {
		...
		row.append(
			$("<td>").html(
				"Wipe Save:<br/><span class='smallDesc'>(Completely wipe your main save and reset the game. COMPLETELY. FOREVER.)</span>",
			),
		);
		row.append(
			$("<td>").append(
		...
		if (SharkGame.persistentFlags.unlockedDebug) {
			...
			row.append(
				$("<td>").html(
					"Hide Cheats:<br/><span class='smallDesc'>(Hide or show cheats.)</span>",
				),
			);
			row.append(
				$("<td>").append(
			...
			optionsTable.prepend(
				$("<tr>").html(
					"<h3><br><span style='text-decoration: underline'>" +
						sharktext.boldString("CHEATS and DEBUG") +
						"</span></h3>",
				),
			);
		}
		...
		row.append(
			$("<td>").html(
				"Keybinds:<br/><span class='smallDesc'>(Change keybinds.)</span>",
			),
		);
		row.append(
			$("<td>").append(
	...
	onOptionClick() {
		...
		const optionIndex = parseInt(settingInfo[2], 10);

		// change setting to specified setting!
		SharkGame.Settings.current[settingName] =
			SharkGame.Settings[settingName].options[optionIndex];
	...
	showKeybinds() {
		...
		$.each(SharkGame.Keybinds.keybinds, (boundKey, boundAction) => {
			...
			if (SharkGame.Keybinds.actions.includes(boundAction)) {
				...
				row.append(
					$("<td>").html(SharkGame.Keybinds.cleanActionID(boundAction)),
				);
			}
	...
	showHelp() {
		const helpDiv = $("<div>");
		helpDiv.append(
			$("<div>").append(SharkGame.Panes.help).addClass("paneContentDiv"),
		);

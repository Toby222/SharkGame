SharkGame.Keybinds = {
	...
	cleanActionID(actionID) {
		if (!this.actions.includes(actionID)) {
			if (SharkGame.HomeActions.getActionTable()[actionID]) {
				...
				actionID = SharkGame.HomeActions.getActionData(
				...
				_.each(SharkGame.HomeActions, (homeActionsObject) => {
					if (
						typeof homeActionsObject === "object" &&
						homeActionsObject[actionID]
					) {
	...
	cleanID(keyID) {
		...
		keyID = keyID
			.replace("Up Page", "Page Up")
			.replace("Down Page", "Page Down");
		return keyID;
	},
	...
	composeKeys(keyID) {
		_.each(["Shift", "Alt", "Control"], (modifier) => {
			...
			if (
				this.modifierKeys[modifier + "Left"] ||
				this.modifierKeys[modifier + "Right"]
			) {
	...
	handleUpBind(actionType) {
		if (!this.tempDisableBind) {
			switch (actionType) {
				default:
					if (
						SharkGame.HomeActions.getActionData(
							SharkGame.HomeActions.getActionTable(),
							actionType,
						) &&
			...
			switch (actionType) {
				...
				case "buy topmost upgrade":
					if (!cad.pause && !cad.stop) {
						SharkGame.Lab.onLabButton(
							SharkGame.Lab.findAllAffordableUpgrades()[0],
						);
					}
					break;
				case "press all buying buttons":
					if (!SharkGame.flags.pressedAllButtonsThisTick) {
						_.each(home.buttonNamesList, (actionName, actionData) => {
							...
							actionData = SharkGame.HomeActions.getActionData(
								SharkGame.HomeActions.getActionTable(),
								actionName,
							);
							if (
								!home.doesButtonGiveNegativeThing(actionData) &&
								home.shouldHomeButtonBeUsable(actionData) &&
								!actionData.isRemoved
							) {
	...
	updateBindModeOverlay(toggledByKey) {
		if (this.bindMode) {
			...
			if (_.isUndefined(this.settingAction) && _.isUndefined(this.settingKey)) {
				textConatiner.append(
					$("<p>").html(
						"<strong>Click the button you want to bind, then press a key to bind it to.</strong>",
					),
				);
			} else if (
				!_.isUndefined(this.settingAction) &&
				_.isUndefined(this.settingKey)
			) {
				textConatiner.append(
					$("<p>").html(
						`<strong>Press a key to bind to ${this.cleanActionID(this.settingAction)}.</strong>`,
					),
				);
			} else if (
				_.isUndefined(this.settingAction) &&
				...
				textConatiner.append(
					$("<p>").html(
						`<strong>Click a button to bind to ${this.settingKey}.</strong>`,
					),
				);
			} else {
				textConatiner.append(
					$("<p>").html(
						`<strong>Bound ${this.settingKey} to ${this.cleanActionID(this.settingAction)}.</strong>`,
					),
				);
			}
	...
	toggleBindMode(toggledByKey) {
		...
		if (
			SharkGame.PaneHandler.isStackClosable() &&

SharkGame.Save = {
	...
	saveGame() {
		...
		saveData.completedRequirements = _.cloneDeep(
		...
		saveData.memories.persistent = _.cloneDeep(
			SharkGame.Memories.persistentMemories,
		);
		...
		saveString = ascii85.encode(
			pako.deflate(JSON.stringify(saveData), { to: "string" }),
		);
	...
	decodeSave(saveDataString) {
		...
		if (saveDataString.charAt(0) === "{") {
			try {
				...
				throw new Error(
					"Couldn't load save data. It didn't parse correctly. Your save: " +
						saveDataString,
				);
			}
		}
	...
	loadGame(importSaveData) {
		...
		const saveDataString =
			importSaveData || localStorage.getItem(SharkGame.Save.saveFileName);
		...
		if (saveData) {
			...
			if (!_.has(saveData, "saveVersion")) {
				...
				typeof saveData.saveVersion !== "number" ||
				...
				throw new Error(
					"This is a save from before New Frontiers 0.2, after which the save system was changed.",
				);
			...
			$.each(saveData.resources, (resourceId, resource) => {
				...
				if (SharkGame.PlayerResources.has(resourceId)) {
					SharkGame.PlayerResources.get(resourceId).amount = Number.isNaN(
						resource.amount,
					)
						? 0
						: resource.amount;
					SharkGame.PlayerResources.get(resourceId).totalAmount = Number.isNaN(
			...
			if (saveData.memories) {
				...
				mem.persistentMemories =
					saveData.memories.persistent || mem.persistentMemories;
			}
			...
			if (saveData.completedRequirements) {
				SharkGame.Gate.completedRequirements = _.cloneDeep(
					saveData.completedRequirements,
				);
			}
			...
			$.each(saveData.settings, (settingId, currentvalue) => {
				...
				if (
					SharkGame.Settings[settingId] &&
					typeof SharkGame.Settings[settingId].onChange === "function"
				) {
	...
	getTaggedSaveCharacteristics(tag) {
		if (_.isUndefined(tag)) {
			SharkGame.Log.addError(
				"Tried to get characteristics of a tagged save, but no tag was given.",
			);
			throw new Error(
				"Tried to get characteristics of a tagged save, but no tag was given.",
			);
		}
		const save = this.decodeSave(
			localStorage.getItem(SharkGame.Save.saveFileName + tag),
		);
		let text;
		if (save) {
			...
			SharkGame.Log.addError(
				`Tried to get characteristics of ${SharkGame.Save.saveFileName + tag}, but no such save exists.`,
			);
			throw new Error(
				`Tried to get characteristics of ${SharkGame.Save.saveFileName + tag}, but no such save exists.`,
			);
	...
	createTaggedSave(tag) {
		if (_.isUndefined(tag)) {
			SharkGame.Log.addError(
				"Tried to create a tagged save, but no tag was given.",
			);
			throw new Error("Tried to create a tagged save, but no tag was given.");
		}
		localStorage.setItem(
			SharkGame.Save.saveFileName + tag,
			localStorage.getItem(SharkGame.Save.saveFileName),
		);
	},
	...
	loadTaggedSave(tag) {
		if (_.isUndefined(tag)) {
			SharkGame.Log.addError(
				"Tried to load a tagged save, but no tag was given.",
			);
		...
		if (this.savedGameExists(tag)) {
			...
			SharkGame.Log.addError(
				`Tried to load ${SharkGame.Save.saveFileName + tag}, but no such save exists.`,
			);
		}
	},
	...
	saveUpdaters: [
		...
		function update0(save) {
			...
			[
				...
				save.resources[resourceName] = { amount: null, totalAmount: null };
			});
			save.upgrades = {};
		...
		function update13(save) {
			_.each(
				["historian", "crimsonCombine", "kelpCultivator"],
				(resourceName) => {
		...
		function update14(save) {
			...
			_.each(save.resources, (resource, resourceId) => {
				if (
					[0, null].includes(resource.amount) &&
					[0, null].includes(resource.totalAmount)
				) {
		...
		function update18(save) {
			if (
				save.resources.essence &&
				save.resources.essence.totalAmount > 0 &&
				save.aspects.apotheosis
			) {

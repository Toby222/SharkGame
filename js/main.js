...
window.onmousemove = (event) => {
	...
	tooltip.style.top =
		Math.max(
			Math.min(posY - 20, window.innerHeight - tooltip.offsetHeight - 10),
			20,
		) + "px";
...
$.extend(SharkGame, {
	...
	changeSprite(spritePath, imageName, imageDiv, backupImageName) {
		...
		if (spriteData) {
			...
			imageDiv.css(
				"background-position",
				"-" + spriteData.frame.x + "px -" + spriteData.frame.y + "px",
			);
...
SharkGame.Main = {
	...
	init() {
		...
		if (isSafari) {
			...
			SharkGame.PaneHandler.addPaneToStack(
				"Safari Notice",
				SharkGame.Panes.safariNotice,
			);
		}
	},
...
Mod of v ${SharkGame.ORIGINAL_VERSION}`,
		...
		$.getJSON(
			"https://api.github.com/repos/Toby222/SharkGame/commits/dev",
			(data) => {
	...
	setUpGame() {
		...
		if (!SharkGame.persistentFlags.dialSetting)
		...
		if (SharkGame.persistentFlags.pause) {
			...
			if (
				SharkGame.flags.needOfflineProgress &&
				SharkGame.Settings.current.truePause
			) {
				SharkGame.persistentFlags.currentPausedTime +=
					SharkGame.flags.needOfflineProgress * 1000;
			...
			if (SharkGame.Settings.current.idleEnabled) {
				...
				if (SharkGame.Aspects.overtime.level) {
					res.minuteHand.updateMinuteHand(
						secondsElapsed * 200 * SharkGame.Aspects.overtime.level,
					);
				...
				main.processSimTime(
					secondsElapsed / SharkGame.persistentFlags.dialSetting,
					true,
				);
			}
	...
	tick() {
		...
		if (cad.pause) {
			if (SharkGame.Settings.current.truePause) {
				...
				res.minuteHand.updateMinuteHand(
					elapsedTime * (1 + SharkGame.Aspects.overtime.level * 0.2),
				);
				res.minuteHand.addBonusTime(
					elapsedTime * SharkGame.Aspects.overtime.level * 0.2,
				);
			}
			SharkGame.before = now;
			SharkGame.lastActivity = now;
		...
		if (!SharkGame.gameOver) {
			...
			if (
				now - SharkGame.lastActivity > SharkGame.IDLE_THRESHOLD &&
			...
			if (res.minuteHand.active) {
				...
				res.minuteHand.updateMinuteHand(
					elapsedTime * 0.2 * SharkGame.Aspects.overtime.level,
				);
				res.minuteHand.addBonusTime(
					elapsedTime * 0.2 * SharkGame.Aspects.overtime.level,
				);
	...
	startIdle(now, elapsedTime) {
		...
		if (idleOverlay.is(":hidden")) {
			...
			idleOverlay
				.show()
				.css("opacity", 0)
				.animate({ opacity: 0.8 }, SharkGame.IDLE_FADE_TIME);
		}
	...
	continueIdle(now, elapsedTime) {
		const speedRatio = Math.min(
			(now - SharkGame.savedMouseActivity - SharkGame.IDLE_THRESHOLD) /
				SharkGame.IDLE_FADE_TIME,
			1,
		);
	...
	createBuyButtons(customLabel, addToWhere, appendOrPrepend, absoluteOnly) {
		if (!addToWhere) {
			log.addError(
				"Attempted to create buy buttons without specifying what to do with them.",
			);
			return;
		}
		...
		switch (appendOrPrepend) {
			...
			default:
				log.addError(
					"Attempted to create buy buttons without specifying whether to append or prepend.",
				);
				return;
		}
		_.each(SharkGame.Settings.buyAmount.options, (amount) => {
			...
			$("#buy-" + amount)
				...
				.on("click", function callback() {
					...
					SharkGame.Settings.current.buyAmount =
						amount === "custom"
							? "custom"
							: parseInt(thisButton.attr("id").slice(4), 10);
		...
		document
			...
			.addEventListener("input", main.onCustomChange);
...
SharkGame.Changelog = {
	...
	"<a href='https://github.com/Toby222/SharkGame'>New Frontiers</a> 0.2 patch 20210515a":
		["Added missing flavor text.", "Internal stuff."],

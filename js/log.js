SharkGame.Log = {
		...
		switch (SharkGame.Settings.current.logLocation) {
			case "left":
				$("#sidebar").append(
					logDiv
						.append("<h3>Log<h3/>")
						.append($("<ul id='messageList'></ul>").addClass("forLeftSide")),
				);
			...
			case "top":
				...
				logDiv
					.append(
						$("<button id='extendLog' class='min close-button'>⯆</button>").on(
							"click",
							log.toggleExtendedLog,
						),
					)
				...
				break;
			default:
				$("#rightLogContainer").append(
					logDiv
						.append("<h3>Log<h3/>")
						.append($("<ul id='messageList'></ul>").addClass("forRightSide")),
				);
				$("#wrapper").removeClass("topLogActive");
				$("#titlebackground").removeClass("topLogActive");
				$("#rightLogContainer")
	...
	changeHeight() {
		if (
			SharkGame.Settings.current.logLocation !== "left" &&
			SharkGame.Settings.current.logLocation !== "top"
		) {
			const maxHeight =
	...
	addMessage(message, sanitizeHtml = false) {
		...
		if (messageList.scrollTop !== 0) {
			...
			const desiredTopElt =
				log.messages[_.clamp(highestVisible + 1, log.messages.length - 1)][0];

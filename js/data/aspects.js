...
SharkGame.Aspects = {
	...
	coordinatedCooperation: {
		...
		getEffect(level) {
			return (
				"Tokens increase production by <strong>" + (level + 2) + "×</strong>."
			);
		},
		getUnlocked() {
			return gateway.completedWorlds.includes("frigid")
				? ""
	...
	cleanSlate: {
		...
		apply(when) {
			if (
				when === "levelUp" &&
				SharkGame.Settings.current.doAspectTable === "table"
			) {
	...
	internalCalculator: {
		...
		getUnlocked() {
			return gateway.completedWorlds.includes("abandoned")
				? ""

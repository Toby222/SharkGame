...
SharkGame.TabHandler = {
			...
			if (reqsMet) {
				...
				switch (tabName) {
					case "reflection":
						...
						break;
					case "cheats":
						if (!SharkGame.persistentFlags.seenCheatsTab)
							log.addDiscovery("Discovered " + tab.name + "!");
						break;
					default:
						log.addDiscovery("Discovered " + tab.name + "!");
	...
	validateTabWidth() {
		...
		if (logLocation !== "left" && logLocation !== "top") {
			$("#tabList").css(
				"margin-right",
				$(window).width() -
					document.getElementById("content").getBoundingClientRect().right +
					14 +
					"px",
			);
		}
	...
	discoverTab(tab) {
		...
		if (
			(tab === "reflection" && SharkGame.persistentFlags.seenReflection) ||
			(tab === "cheats" && SharkGame.persistentFlags.seenCheatsTab)
		) {
			SharkGame.Tabs[tab].seen = true;
		}

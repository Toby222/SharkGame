...
SharkGame.Events = {
	...
	volcanicHandleAlgaeSpongeRelationship: {
		...
		trigger() {
			...
			const limitRatio = SharkGame.Upgrades.purchased.includes(
				"feedingTechniques",
			)
				? 4
				: 1;
			...
			$.each(SharkGame.flags.storm, (resourceName, removalRatio) => {
				if (world.doesResourceExist(resourceName)) {
					storm.baseIncome[resourceName] =
						(predictedTimeUntilNextTick *
							res.getResource(resourceName) *
							removalRatio) /
						(1 - predictedTimeUntilNextTick * removalRatio);
				}
	...
	tempestuousFindCave: {
		...
		trigger() {
			...
			_.each(
				["fish", "sand", "crystal", "shark", "ray", "crab"],
				(resourceName) => {
	...
	tempestuousEmergencySeagrass: {
		...
		getAction() {
			if (SharkGame.World.worldType !== "tempestuous") {
				...
				SharkGame.flags.gaveSeagrass &&
				res.getResource("seagrass") < 10 &&
				res.getResource("stormgoer") < 1
			) {
				return "trigger";
			}
			return "pass";
	...
	tempestuousMapSequence: {
		...
		getAction() {
			if (SharkGame.World.worldType !== "tempestuous") {
				...
				SharkGame.Upgrades.purchased.includes("cartographicCompleteness")
			) {
				return "trigger";
			}
			return "pass";

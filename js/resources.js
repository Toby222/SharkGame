...
SharkGame.Resources = {
	...
	init() {
			...
			if (resourceObject.name) {
				Object.defineProperty(
					...
					Object.getOwnPropertyDescriptor(
						SharkGame.ResourceTable[resourceId],
						"name",
					),
				);
			}
			...
			if (resource.income) {
				...
				Object.defineProperties(
					...
					Object.getOwnPropertyDescriptors(
						SharkGame.ResourceTable[resourceId].income,
					),
				);
			}
		...
		SharkGame.ResourceIncomeAffectors = _.cloneDeep(
			SharkGame.ResourceIncomeAffectorsOriginal,
		);
		SharkGame.GeneratorIncomeAffectors = _.cloneDeep(
	...
	processIncomes(timeDelta, debug, simulatingOffline) {
		...
		while (timeDelta > 1) {
			...
			SharkGame.PlayerIncomeTable.forEach((income, resourceId) => {
				if (
					!SharkGame.ResourceSpecialProperties.timeImmune.includes(resourceId)
				) {
	...
	recalculateIncomeTable(cheap) {
				...
				if (resource.income) {
					...
					$.each(resource.income, (generatedResource) => {
						if (world.doesResourceExist(generatedResource)) {
							changeMap.set(
								generatedResource,
								res.getProductAmountFromGeneratorResource(
									resourceId,
									generatedResource,
								),
							);
					...
					changeMap.forEach((generatedAmount, generatedResource) => {
						...
						if (generatedAmount > 0) {
							SharkGame.PlayerResources.get(generatedResource).discovered =
								true;
						}
					});
					...
					if (!cheap) {
						...
						changeMap.forEach((amount, generatedResource) => {
							...
							SharkGame.FlippedBreakdownIncomeTable.get(generatedResource)[
								resourceId
							] = income;
							SharkGame.PlayerIncomeTable.set(
								generatedResource,
								SharkGame.PlayerIncomeTable.get(generatedResource) + income,
							);
						});
						...
						changeMap.forEach((amount, generatedResource) => {
							SharkGame.PlayerIncomeTable.set(
								generatedResource,
								SharkGame.PlayerIncomeTable.get(generatedResource) +
									amount * costScaling,
							);
						});
					}
		...
		$.each(
			SharkGame.ResourceSpecialProperties.incomeCap,
			(resourceId, maxProduction) => {
	...
	getProductAmountFromGeneratorResource(
		generator,
		product,
		numGenerator = res.getResource(generator),
	) {
		...
		if (node) {
			if (node.multiply) {
				$.each(node.multiply, (resourceOrGroup, amount) => {
					multiplier =
						multiplier * (1 + amount * res.getResource(resourceOrGroup));
				});
			}
			if (node.reciprocate) {
			...
			if (node.exponentiate) {
				$.each(node.exponentiate, (resourceOrGroup, amount) => {
					multiplier = multiplier * amount ** res.getResource(resourceOrGroup);
				});
			}
			if (node.polynomial) {
				$.each(node.polynomial, (resourceOrGroup, amount) => {
					multiplier =
						multiplier * (1 + res.getResource(resourceOrGroup) ** amount);
				});
			}
		}
		return multiplier;
	...
	getGameSpeedModifier() {
		return (
			(cad.speed * res.idleMultiplier) / SharkGame.persistentFlags.dialSetting
		);
	},
	...
	isCategoryVisible(category) {
		return (
			...
			_.some(category.resources, (resourceName) => {
				...
				return (
					resource &&
					(resource.totalAmount > 0 || resource.discovered) &&
					world.doesResourceExist(resourceName)
				);
			})
		);
	},
	...
	getResourcesInCategory(categoryName) {
		const resources = [];
		SharkGame.ResourceCategories[categoryName].resources.forEach(
			(resourceName) => {
	...
	isInCategory(resource, category) {
		return (
			SharkGame.ResourceCategories[category].resources.indexOf(resource) !== -1
		);
	},
	...
	checkResources(resourceList, checkTotal) {
		return _.every(resourceList, (required, resource) => {
			const currentAmount = checkTotal
				? res.getTotalResource(resource)
				: res.getResource(resource);
			if (typeof required === "object") {
				return required.lessThanOrEqualTo(
					currentAmount * (1 + SharkGame.EPSILON),
				);
			}
	...
	tokens: {
		...
		setup() {
			...
			_.each(this.list, (token) => {
				...
				if (
					...
					SharkGame.flags.tokens[token.attr("id")] !== "RETURNME" &&
					world.doesResourceExist(
						SharkGame.flags.tokens[token.attr("id")].split("-")[1],
					)
				) {
					res.tokens.markLocation(
						token.attr("id"),
		...
		makeToken(type = "nobody cares", initialLocation = "NA") {
			...
			const token = SharkGame.changeSprite(
			...
			if (SharkGame.Settings.current.showTooltips) {
				if (SharkGame.flags.tokens[this.id] === "NA") {
					...
					$("#tooltipbox")
						.html(
							sharktext.boldString(
								"Click this slot or click the token to recall it.",
							),
						)
						.addClass("forHomeButtonOrGrotto");
				}
		...
		handleTokenDragStart(event) {
			...
			event.originalEvent.dataTransfer.setData(
				"tokenLocation",
				SharkGame.flags.tokens[this.id],
			);
		...
		updateColorfulDropZones() {
			SharkGame.ResourceMap.forEach((_resourceData, resourceName) => {
				if (
					...
					if (
						!$.isEmptyObject($("#resource-" + resourceName)) &&
						res.tokens.canBePlacedOn("resource-" + resourceName)
					) {
						$("#resource-" + resourceName).addClass("highlightedResource");
					}
					if (
						!$.isEmptyObject($("#income-" + resourceName)) &&
						res.tokens.canBePlacedOn("income-" + resourceName)
					) {
			...
			if (SharkGame.Settings.current.verboseTokenDescriptions) {
				...
				_.each(res.tokens.list, (token) => {
					...
					if (tokenLocation === "NA") {
						...
						textToDisplay +=
							"boosting all " +
							sharktext.getResourceName(tokenLocation.split("-")[1], false, 1) +
							" gains.";
		...
		dropToken(event) {
			...
			const originalTokenId =
				event.originalEvent.dataTransfer.getData("tokenId");
			const previousLocation =
				event.originalEvent.dataTransfer.getData("tokenLocation");
		...
		applyTokenEffect(targetId, _id, reverseOrApply = "apply") {
			const multiplier =
				...
				(SharkGame.Aspects.collectiveCooperation.level + 1);
			...
			if (targetId.includes("resource")) {
				res.applyModifier(
					...
					targetId.split("-")[1],
					reverseOrApply === "apply" ? multiplier : 1 / multiplier,
				);
				...
				SharkGame.Log.addError(
					"Tried to apply token effect, but couldn't understand the location!",
				);
			...
			if (placedOnWhat.includes("resource")) {
				return (
					...
					_.some(
						...
						(amount, generatedResource) =>
							amount !== 0 && world.doesResourceExist(generatedResource),
					)
				);
	...
	minuteHand: {
		...
		init() {
			...
			this.active = false;
			this.changeSelectedMultiplier(
				null,
				SharkGame.persistentFlags.selectedMultiplier,
			);
		...
		buildUI() {
			...
			$("#minute-hand-div").append(
				$("<div>").attr(
					"id",
					"minute-time",
				) /* .on("mouseenter", res.minuteHand.showTimeTooltip).on("mouseleave", res.tableTextLeave) */,
			);
		...
		updateMinuteHand(timeElapsed) {
			...
			if (res.minuteHand.disableNextTick) {
				...
				SharkGame.flags.minuteHandTimer +=
					timeElapsed * (1 + SharkGame.Aspects.doubleTime.level);
				res.minuteHand.addBonusTime(
					timeElapsed * SharkGame.Aspects.doubleTime.level,
				);
			} else {
				...
				if (SharkGame.flags.minuteHandTimer < 0) {
					...
					res.minuteHand.changeRealMultiplier(
						SharkGame.flags.minuteHandTimer / timeElapsed +
							res.minuteHand.realMultiplier -
							1,
					);
					SharkGame.flags.minuteHandTimer = 0;
					res.minuteHand.toggleMinuteHand();
		...
		toggleMinuteHand() {
			if (!res.minuteHand.active && SharkGame.flags.minuteHandTimer > 0) {
				...
				log.addMessage(
					"<span class='minuteOn'>" +
						SharkGame.choose(res.minuteHand.onMessages) +
						"</span>",
				);
			} else if (res.minuteHand.active) {
				...
				log.addMessage(
					"<span class='minuteOff'>" +
						SharkGame.choose(res.minuteHand.offMessages) +
						"</span>",
				);
			}
		...
		changeSelectedMultiplier(_event, arbitrary) {
			...
			if (arbitrary) {
				...
				multiplier =
					2 ** Math.floor(document.getElementById("minute-slider").value);
			}
		...
		updateMinuteHandLabel() {
			const multiplier = SharkGame.persistentFlags.selectedMultiplier
				.toString()
				.padStart(3, " ")
				.replaceAll(" ", "&nbsp;&nbsp;");
			$("#minute-multiplier").html(
				"<span class='click-passthrough bold'>" + multiplier + "×</span>",
			);
			$("#minute-time").html(
				sharktext.boldString(
					"(" +
						res.minuteHand.formatMinuteTime(SharkGame.flags.minuteHandTimer) +
						")",
				),
			);
		...
		giveRequestedTime() {
			if (SharkGame.persistentFlags.requestedTime) {
				SharkGame.flags.minuteHandTimer +=
					SharkGame.persistentFlags.requestedTime;
				this.addBonusTime(SharkGame.persistentFlags.requestedTime);
				SharkGame.flags.requestedTimeLeft =
					SharkGame.persistentFlags.requestedTime;
				SharkGame.Log.addMessage(
					`Took ${this.formatMinuteTime(SharkGame.persistentFlags.requestedTime)} out of storage.`,
				);
		...
		formatMinuteTime(milliseconds, alwaysRoundSeconds) {
			...
			const formatSeconds =
				(numSeconds >= 60 || alwaysRoundSeconds
					? Math.round(numSeconds % 60)
					: (numSeconds % 60).toFixed(1)) + "s";
		...
		updatePowers() {
			...
			$("#minute-hand-toggle")
				...
				.removeClass("power5");
			if (
				res.minuteHand.active &&
				SharkGame.Settings.current.minuteHandEffects
			) {
				const multiplier =
					2 ** Math.floor(document.getElementById("minute-slider").value);
				if (multiplier === 2) {
	...
	pause: {
		...
		showTooltip() {
			if (cad.pause) {
				...
				$("#tooltipbox").html(
					"Click to <strong>pause</strong>, stopping most timers and all resources.",
				);
			}
		},
	},
	...
	reconstructResourcesTable() {
		...
		if (SharkGame.Settings.current.groupResources) {
				...
				if (res.isCategoryVisible(category)) {
						...
						.append(
							$("<td>")
								...
								.append(
									$("<h3>")
										.html(
											`<span class="collapser">${icon}</span><span>${categoryName}</span>`,
										)
										.css("text-align", "left"),
								),
						)
						.on("click", () =>
							SharkGame.Resources.collapseResourceTableRow(categoryName),
						);
					...
					_.each(category.resources, (resource) => {
						if (
							...
							(SharkGame.PlayerResources.get(resource).discovered &&
			...
			SharkGame.ResourceMap.forEach((_resource, resourceId) => {
				if (
					(res.getTotalResource(resourceId) > 0 ||
						SharkGame.PlayerResources.get(resourceId).discovered) &&
					world.doesResourceExist(resourceId) &&
					res.isCategoryVisible(
						SharkGame.ResourceCategories[res.getCategoryOfResource(resourceId)],
					)
				) {
	...
	constructResourceTableRow(resourceKey) {
		...
		const row = $("<tr>")
			...
			.on("mouseleave", res.tableTextLeave);
		if (
			playerResources.totalAmount > 0 ||
			SharkGame.PlayerResources.get(resourceKey).discovered
		) {
			row.append(
				$("<td>")
					...
					.html(
						sharktext.getResourceName(
							...
							undefined,
							undefined,
							sharkcolor.getVariableColor("--color-darker"),
						),
					)
					.on("dragstart", res.tokens.handleResourceDragStart)
					.on("dragover", (event) => {
						if (
							res.tokens.canBePlacedOn("resource-" + resourceKey) &&
							res.tokens.chromeForcesWorkarounds
						) {
							if (SharkGame.Settings.current.showTooltips) {
								$("#tooltipbox").html(
			...
			const incomeId = $("<td>")
				...
				.on("dragover", (event) => {
					if (
						res.tokens.canBePlacedOn("income-" + resourceKey) &&
						res.tokens.chromeForcesWorkarounds
					) {
						if (SharkGame.Settings.current.showTooltips) {
							$("#tooltipbox").html(
	...
	tableTextEnter(_mouseEnterEvent, resourceName) {
		...
		let isConsumingText = "";
		$.each(
			SharkGame.BreakdownIncomeTable.get(resourceName),
			(generatedResource, amount) => {
		...
		$.each(generators, (which, amount) => {
			if (Math.abs(amount) > SharkGame.EPSILON) {
				if (amount > 0) {
					producertext += "<br>";
					producertext +=
						(sharktext.shouldHideNumberOfThis(which)
							? ""
							: "<strong>" +
								sharktext.beautify(res.getResource(which)) +
								"</strong> ") +
						sharktext.getResourceName(
							which,
							false,
			...
			switch (type) {
				case "increase":
					$.each(effects, (affectedGenerator, degree) => {
						furtherCondensedEffects.generators.increase[affectedGenerator] =
							degree;
					});
					break;
				case "decrease":
					$.each(effects, (affectedGenerator, degree) => {
						furtherCondensedEffects.generators.decrease[affectedGenerator] =
							-degree;
					});
					break;
				case "multincrease":
					$.each(effects, (affectedGenerator, degree) => {
						if (
							typeof furtherCondensedEffects.generators.increase[
								affectedGenerator
							] !== "number"
						) {
							furtherCondensedEffects.generators.increase[affectedGenerator] =
				...
				case "multdecrease":
					$.each(effects, (affectedGenerator, degree) => {
						if (
							typeof furtherCondensedEffects.generators.decrease[
								affectedGenerator
							] !== "number"
						) {
							furtherCondensedEffects.generators.decrease[affectedGenerator] =
			...
			switch (type) {
				case "increase":
					$.each(effects, (affectedResource, degree) => {
						furtherCondensedEffects.resources.increase[affectedResource] =
							degree;
					});
					break;
				case "decrease":
					$.each(effects, (affectedResource, degree) => {
						furtherCondensedEffects.resources.decrease[affectedResource] =
							-degree;
					});
					break;
				case "multincrease":
					$.each(effects, (affectedResource, degree) => {
						if (
							typeof furtherCondensedEffects.resources.increase[
								affectedResource
							] !== "number"
						) {
							furtherCondensedEffects.resources.increase[affectedResource] =
				...
				case "multdecrease":
					$.each(effects, (affectedResource, degree) => {
						if (
							typeof furtherCondensedEffects.resources.decrease[
								affectedResource
							] !== "number"
						) {
							furtherCondensedEffects.resources.decrease[affectedResource] =
		...
		$.each(
			furtherCondensedEffects.generators.increase,
			(affectedGenerator, degree) => {
				if (
					world.doesResourceExist(affectedGenerator) &&
					res.getResource(affectedGenerator) > SharkGame.EPSILON
				) {
					increaseText += "<br>";
					increaseText +=
						sharktext.getResourceName(
						...
						" speed by " +
						sharktext.boldString(
							`${sharktext.beautify(Math.floor(100 * degree))}%`,
						);
				}
			},
		);
		...
		$.each(
			furtherCondensedEffects.resources.increase,
			(affectedResource, degree) => {
				if (
					world.doesResourceExist(affectedResource) &&
					res.getResource(affectedResource) > SharkGame.EPSILON
				) {
					increaseText += "<br>";
					increaseText +=
						sharktext.getResourceName(
		...
		$.each(
			furtherCondensedEffects.generators.decrease,
			(affectedGenerator, degree) => {
		...
		$.each(
			furtherCondensedEffects.resources.decrease,
			(affectedResource, degree) => {
				if (degree < 0) degree = 1 + degree;
				if (
					world.doesResourceExist(affectedResource) &&
		...
		let text = sharktext.getResourceName(
			resourceName,
			false,
			2,
			sharkcolor.getElementColor("tooltipbox", "background-color"),
		...
		if (isGeneratingText !== "") {
			text +=
				"<br><span class='littleTooltipText'>" +
				sharktext.getIsOrAre(resourceName, 2).toUpperCase() +
				" PRODUCING</span>" +
				isGeneratingText;
		}
		if (isConsumingText !== "") {
		...
		if (
			(isGeneratingText || isConsumingText) &&
			(producertext || consumertext)
		) {
		...
		if (
			...
			(producertext || consumertext || isGeneratingText || isConsumingText)
		) {
			text += "<br><span class='littleTooltipText'>and</span>";
		}
		if (increaseText !== "") {
		...
		if (SharkGame.ResourceMap.get(resourceName).desc) {
			text +=
				"<hr class='hrForTooltipSeparation'><span class='medDesc'>" +
				SharkGame.ResourceMap.get(resourceName).desc +
				"</span>";
		}
		if (
	...
	buildIncomeNetwork() {
		...
		$.each(gia, (affectorResource) => {
			$.each(gia[affectorResource], (type) => {
				$.each(
					gia[affectorResource][type],
					(affectedGeneratorCategory, value) => {
						...
						$.each(nodes, (_key, affectedGenerator) => {
							if (
								world.doesResourceExist(affectedGenerator) &&
								world.doesResourceExist(affectorResource)
							) {
								res.addNetworkNode(
									rgad,
									affectedGenerator,
		...
		$.each(ria, (affectorResource) => {
			$.each(ria[affectorResource], (type) => {
				$.each(
					ria[affectorResource][type],
					(affectedResourceCategory, degree) => {
						// is it a category?
	...
	reapplyModifiers(generator, generated) {
		...
		SharkGame.ModifierReference.forEach((modifier, name) => {
			const generatorDegree =
				SharkGame.ModifierMap.get(generator)[modifier.category][modifier.type][
					name
				];
			const generatedDegree =
				SharkGame.ModifierMap.get(generated)[modifier.category][modifier.type][
					name
				];
			income = modifier.applyToInput(
				income,
				generatorDegree,
				generatedDegree,
				generator,
	...
	getMultiplierProduct(category, generator, generated) {
		...
		$.each(SharkGame.ModifierTypes[category].multiplier, (name, data) => {
			const generatorDegree =
				SharkGame.ModifierMap.get(generator)[data.category][data.type][name];
			const generatedDegree =
				SharkGame.ModifierMap.get(generated)[data.category][data.type][name];
			product *= data.getEffect(
				generatorDegree,
				generatedDegree,
				generator,
				generated,
			...
			if (genNode) {
				$.each(genNode, (type, affectorObjects) => {
					$.each(affectorObjects, (affectedGenerator, degree) => {
						if (
							!returnable.genAffect[convertType(type, degree)][
								affectedGenerator
							]
						) {
							switch (type) {
			...
			if (resNode) {
				$.each(resNode, (type, affectorObjects) => {
					$.each(affectorObjects, (affectedResource, degree) => {
						if (
							!returnable.resAffect[convertType(type, degree)][affectedResource]
						) {
							switch (type) {
								case "multiply":
									returnable.resAffect[convertType(type, degree)][
										affectedResource
									] = amount * degree;
									break;
								case "exponentiate":
